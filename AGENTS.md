...
> Last updated: 2026-09-04 (filesystem overhaul phase B0 landed — missing files are
> graceful everywhere on the read/write/open paths, Get_FileSize(path) works, the
> dead ustring overloads/scratch-cache/std::exit paths are gone, capacity is a
> single 512; see the K25 row).
