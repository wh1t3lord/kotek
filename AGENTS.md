...
> Last updated: 2026-09-23 (K11 phase 3 / zircon Z24 B3b: the geometry
> seam — `ktkIRenderGeometryManager` + the frame-level draw commands on
> `ktkIRenderFramePassContext`; see the K11 row).
...
| K11 | Delete GAPI/render backends except bgfx; add NVIDIA NRI backend (kotek + zircon) | in-progress (2026-07-25 phase 1 done) | DELETED: kotek.render.{gl,vk,software,angle.gles,angle.gles23,gl.glad,shared.gl,shared.vk,shared.dx} + zircon render/{gles3,vk}; bgfx kept & de-GL'd. **NRI phase 1 LIVE (2026-07-25)**: `kotek.render.nri` module (`ktkRenderDevice`/`ktkRenderSwapchain`/`ktkRenderResourceManager` over NRI **v180**, D3D12-only, static) — device+queue+cmd alloc/buffer+fence, swap chain (2 back buffers, vsync from config), clear-color present (acquire→barrier→clear→barrier→submit→present, frame-fence paced); resource manager is an honest phase-1 stub (warns, never aborts). Dep: pinned shallow clone into `external/nri` (gitignored) via `cmake/windows/nri.cmake` (`-DKOTEK_NRI=OFF` skips), which also applies an idempotent patch gating NRI's OMM dummy structs on `D3D12_SDK_VERSION < 615` (SDK 26100=616 ships the real ones → C2011 otherwise). Dispatch: the DirectX slot maps to NRI in `main_render_dll.cpp` init+shutdown (both the `--render_dx*` branch and the config-default `is_dx` branch, `KOTEK_INVOKE_MODULE` so K21 overrides apply). New flag `--render_nri_dx12` (constant in `kotek_std_constants.h`): parser maps it to `DirectX_SpecifiedByUser`+`kDirectX_Latest` and engine-config skips the bgfx startup default when present. rmlui manager accepts the DirectX slot. Tests: `kotek_core_test_render_nri.cpp` (4 tests — flag parse + negative control GPU-free; device lifecycle + 3-frame present on a hidden owned HWND, GPU-gated `GTEST_SKIP`). Zircon side: `zircon_renderer_nri` (minimal `ktkIRenderer`, forwards `draw()` to the swapchain Present; no NRI types in zircon), DirectX-slot cases in all game-manager renderer switches, sessions skip render-graph init. VERIFIED 2026-07-25: full Debug build green; boot `--render_nri_dx12 --no_splash --kotek_frames=30` exit 0 (RTX 4060 Laptop, ID3D12Device8, NRI v180, shader model 609, 1920x1175 2-buffer vsync swap chain, clean shutdown), 189/189 kotek tests + default bgfx boot exit 0 unchanged. **GEOMETRY SEAM DONE (2026-09-23, phase 3 / zircon Z24 B3b)**: the device-level geometry manager — NEW `ktkIRenderGeometryManager` in kotek.core.api (ADDITIVE; next to the swapchain interface): opaque VALUE handles (generation-tagged u32 — `(generation << 16) | (index+1)`, the invalid sentinel 0xFFFFFFFF; no backend pointer ever crosses the boundary) over `Create_Buffer(size, usage bits)/Destroy_Buffer/Upload_Buffer` (CPU->GPU through HOST_UPLOAD memory: SYNCHRONOUS + IMMEDIATE, readable from the next submitted frame, no fence — a copy-queue ring is the documented up-scale) + `Create_Pipeline(desc)/Destroy_Pipeline` (DXIL bytecode PODs, one interleaved vertex stream, the color format as a kotek enum, one optional PUSH-CONSTANT range at register(b0) visible to both stages — D3D12 root 32-bit constants); the FRAME-LEVEL commands live additively on `ktkIRenderFramePassContext`: `Begin_Render_Pass/End_Render_Pass` (the LOAD section over the acquired back buffer), `Set_Pipeline/Set_Push_Constants/Set_Vertex_Buffer/Set_Index_Buffer/Draw_Indexed`, `Get_Back_Buffer_Width/Height`. Stale/foreign handles and out-of-order calls are LOUD NO-OPs per command, never crashes (the mid-frame guard). `ktkMainManager` gained the `Set_/GetRenderGeometryManager` slot. IMPLEMENTATION (kotek.render.nri): NEW `ktkRenderGeometryManager` + the NRI-type-free `ktkRenderGeometryHandleTable<Capacity>` header (pure bookkeeping — generation reuse, the capacity + upload-range contracts — proven headlessly by `KotekRenderNRI.GeometryHandleTableBookkeeping` in kotek_core_test_render_nri.cpp, included cross-module WITHOUT a link edge on purpose); buffers = CreateBuffer + GetBufferMemoryDesc(HOST_UPLOAD) + AllocateMemory + BindBufferMemory; pipelines = one-root-constant PipelineLayout + GraphicsPipelineDesc (cull NONE in v1 — B3c owns culling; CCW front documented). Registered in the module entry after the device exists, destroyed (Shutdown releases every live slot) before the device. PIPELINE-CHOICE NOTE: the vendored NRI v180 implements mesh shaders INTERNALLY (PipelineD3D12 consumes MESH_SHADER; DrawMeshTasks exists in CommandBufferD3D12) but the PUBLIC CoreInterface table exposes NO mesh-dispatch entry — and vendored third-party stays byte-pristine — so the B3b first draw is the CLASSIC indexed draw; the seam shape is identical for the future mesh path (a Draw_Meshlets lands with an NRI version bump, B3c). Boot proof: the zircon meshlet pass creates real buffers/pipeline and 30 frames draw exit 0 (zircon Z24 row). REMAINING (phase 2+): zircon passes split for NRI (Z5), NRI-Vulkan slot, ray tracing (the RTX hardware + NRI RT extension are ready), swap-chain resize hookup, user-supplied backend registration |
...
| K25 | Filesystem overhaul: B0 foundation repair → B1 VFM-mapped reads → B2 packed archives (.kpack) → B3 streaming/async → B4 embedded fault-tolerant defaults | done (2026-09-12 — B0/B1/B2a/B2b/B3/B4 ALL landed; the overhaul is COMPLETE) | owner-approved plan 2026-09-04 (session plans dir). **B0 DONE (2026-09-04)**: (1) missing file on ANY native read/write/open path = false / kInvalidFileHandleType + one `KOTEK_MESSAGE_WARNING` with the path — the fopen/exists asserts are gone (user data is not a programmer error); (2) `Get_FileSize(path)` implemented at both levels (native fopen/ftell graceful, full dispatcher boilerplate); (3) interface-dead `ustring` Read/Write_File overloads deleted from `ktkIFileSystem`+`ktkFileSystem` (zero callers in both repos); `Write_File(path, const uint8_t*, size)` is real — forwards to the char* dispatch, native write mode flipped `"w"`→`"wb"` so byte payloads write without CRLF translation (json writers escape raw newlines, nothing observes it — proven by the 0x0A roundtrip test); (4) all 39 `std::exit(-1)` in the dispatch boilerplate → KOTEK_ASSERT + return false/invalid-handle; the 8 "all file systems failed" trailing asserts dropped (the warning after them already degrades); the zlib-shell "todo: implement" asserts on the same paths → graceful skip warnings; (5) the shared-scratch return path deleted (`m_reserved_cache[2048]`) — buffer-too-small = false + the REQUIRED size in the size out-param, the pointer is never redirected (the `ktkIFileSystem::Read_File` contract comment rewritten); (6) the DYN-branch `ktk_array` copy-paste in `kotek_std_alias_filesystem_path.h` fixed to `ktk_filesystem_path`; (7) ~640 commented legacy lines deleted (5 blocks in kotek_filesystem.cpp, the file_text Load/Save block, the "todo: legacy delete" header constants); declared-but-never-defined `ktkFileSystem_Native::Initialize` removed (B1 revives the concept); dead `VFM_FILE_MAX_COUNT` removed; (8) `KOTEK_DEF_FILESYSTEM_STORAGE_MAX_FILES_COUNT` is one value 512 (was 2048/32 on KOTEK_SDK_IMGUI; measured boot handles are single digits, 512 covers B1-B3 headroom). BUGS FOUND+FIXED en route: the dispatch `Open_File` discarded the native handle (returned kInvalidFileHandleType ALWAYS) and its empty-path early return returned 0-as-handle; native `Close_File` never returned the handle to the pool (slot leak); the `repeat_fs` capacity check was INVERTED (`capacity <= list_size`, fired on any explicit-priority call with the priority-list feature — never exercised before) → now `list_size <= capacity`. BUG DOCUMENTED, NOT FIXED (B2 scope, changes runtime behavior): `ktkFrameworkConfig::Set_FS_PriorityList` counts entries `!= kAuto && != kEndOfEnum`, and `kZlib == kEndOfEnum` (=2) — the sentinel collision silently drops the kZlib entry, so the stored priority list for `["Native","ZLIB"]` is `[Native]` (list_size 1); the zlib branches therefore never execute today — fixing it (drop the kEndOfEnum exclusion) activates the zlib-fallback warnings and belongs with B2's real priority-list work. ALSO DOCUMENTED (plan §Risks, not redesigned in B0): the repeat_fs fallback loop is provably unreachable as written (`was_overloaded_fs_order` is mutually exclusive with the loop's guard) — kept for B2; the handle-as-pointer-into-pool ABI; `Read_File(path, char*&)` stays an assert-stub (interface-kept, zero callers); `ktkFileSystem_Zlib::Initialize` declared-never-defined (B2 shell). Tests: 5 new `Filesystem.test_b0_*` suites in kotek_core_test_filesystem.cpp (missing-file graceful read in both dispatch shapes + missing open, Get_FileSize present/absent in both shapes, uint8 write→read byte-exact roundtrip incl. 0x0A, handle Open/GetFileSize/Tell/Seek/Write/Close roundtrip, buffer-too-small false+required-size then retry-with-reported-size) — fixtures under data_user/tests, self-cleaning. VERIFIED: build+gfxdev Debug RC 0 (0 errors, no new compile warnings), boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, zircon 61/61 + kotek 232/232 (227+5), 4 debug_heap(996) residuals (≤6 tolerated), zero new boot-flow warnings (the missing-file warnings fire only from the tests that prove them). Zircon side (Z8): `gltf_read_file` migrated from the Is_Exists+alias-layer file_size TOCTOU pair to the interface `Get_FileSize(path)`; the three shader-blob readers' stale "native read path asserts" comments updated (the Is_Exists guards stay — they suppress the warning); scene_metadata's guard comment didn't cite the assert (untouched). **B1 DONE (2026-09-05)** — VFM-mapped reads are REAL behind the existing `VFM_READ` runtime flag, in the STREAMING-FIRST posture (owner directive 2026-09-05, zircon rule 9: NO user-space mapping cache — the OS page cache already is the file-content cache): the native single-shot read with the flag on maps the file via the NEW `ktkFileSystem_VFM::MapFileForRead` (classifies `eVFMMapFileResult::{kSuccess,kEmptyFile,kMissingFile,kMappingFailed}`), copies out of the mapping in `KOTEK_DEF_FILESYSTEM_VFM_STREAM_CHUNK_SIZE` chunks (64 KB, NEW named define — the consumer never holds more than one chunk over its own buffer), and unmaps immediately (map->copy->unmap, nothing persists; repeated access rides the OS page cache or B3's streaming). Contracts on the mapped path, all pinned by tests: byte-identical to the CRT path INCLUDING the room-permitting terminator at `[size]`; the B0 too-small-buffer contract (false + REQUIRED size, pointer never redirected); missing file = false + size 0 + ONE warning with the CRT fallback deliberately NOT engaged (no double noise); a mapping failure on an existing file warns (VFM-side, with GetLastError) + degrades to the CRT read — never a hard error on user data; a 0-byte file is an explicit successful empty read detected by size query BEFORE any map attempt (CreateFileMapping rejects size 0 on Win32). `VFM_CACHE` is RETIRED: the enum value stays so old configs parse, setting it warns once per filesystem instance (points at the OS page cache + streaming) and is otherwise inert; the shipped sys_info.json drops it (`FS_Features:["VFM_READ","PRIORITY_LIST"]`); the 379-hits-per-boot "vfm cache is not implemented" warning is dead. VFM-layer fixes en route (zero callers before B1, never exercised): `MapFile` returned `size()-1` instead of a reused free slot's id and read the free-id queue `.back()`/`.pop()` (front/back mismatch); `UnMapFile` ERASED its slot (positions of other live ids shifted under any holder of an id) — now marks free + returns the id to the pool, positions stable; `MapViewOfFile` failure leaked the mapping handle (now closed); the per-mapping debug byte-preview at Shutdown and the per-unmap debug line became one summary message with the counters. Wiring: `ktkFileSystem::Initialize` calls `m_vfm.Initialize()` + `m_fs_native.Initialize(&m_vfm)` (the B1-revived native Initialize, VFM-guarded). Diagnostics (house pattern, test-pinned): `Get_StatMapCount()/Get_StatUnmapCount()` on the VFM + `ktkFileSystem::Get_VFM()` concrete-class seam (deliberately NOT on ktkIFileSystem); outstanding = maps − unmaps, every read balances, Shutdown must see zero. THREADING: NOT thread-safe by design this phase (the filesystem is single-threaded today; the zircon resource worker's real IO is B3's scope — documented on the class). The `file_handle_vfm_t` variant stays unconstructed (the single-shot path needs no handles; B3 decides). Tests: 5 NEW `Filesystem.test_b1_*` suites in kotek_core_test_filesystem.cpp (fixtures in data_user/tests, self-cleaning; flags driven through the test's own `ktkFrameworkConfig::Set_FS_FeaturesFlag` AFTER Initialize because Initialize parses the shipped sys_info.json into the config): vfm==native byte parity on 4 KB binary incl 0x00/0xFF across CRT/mapped/retired-flag shapes + the too-small contract on the mapped path; a 200,011-byte file (3 full 64 KB chunks + a 3,403-byte tail) byte-identical through the chunked copy with map/unmap balance pins; missing file graceful in both flag shapes with counters proving nothing mapped; 0-byte file success + terminator parity + no map; shutdown balance (maps==unmaps at rest and after Shutdown). VERIFIED: build+gfxdev Debug RC 0, boot `--no_splash --editor_imgui --kotek_frames=30` exit 0 with zero "vfm cache is not implemented" lines and zero new warnings, a flags-off boot exercises the CRT path (exit 0), zircon + kotek suites green, debug_heap(996) residuals within the tolerated ≤6 | B2a DONE (2026-09-12): the `.kpack` format v1 + the reader backend (NEW module `kotek.core.filesystem.pack`). Format (spec in `kotek_kpack_format.h`): 20-byte header (magic `KPACK01\0` carrying the version + entry_count + reserved flags), 45-byte entry records (name_hash u64 — fnv1a-64 over the fold rule ('\\'→'/', ASCII fold to lower) of the root-relative path — one source for writer and reader; data_offset u64 absolute 4 KB-aligned; raw_size/packed_size u64; compression u8 = stored|zstd|zlib; block_count u32 = ceil(raw/64 KB); block_table_offset u64), per-entry block tables (packed_block_size u32 each), data blocks in writer (load) order. Mount reads the tables ONCE into bounded member storage (caps `KOTEK_DEF_FILESYSTEM_PACK_MAX_ENTRIES`=4096 clamped-loud / `..._MAX_BLOCKS_PER_ENTRY` / `..._MAX_BLOCKS_TOTAL`, reject-loud on excess); every read decompresses ONLY the 64 KB blocks the requested range spans (streaming posture; `Read_File_Block` is B3's stream step). Read tri-state: success / kNotFound (SILENT fallthrough — the pack is an override-chain backend) / kTooSmall (B0 required-size contract) / kCorrupt (loud at detection). `kpack_write_file` is THE encoder B2b's `zircon_kpacker` links (block-streamed write through fixed scratch, in-place table patch at the end, partial-file-fails-mount on error). Mounting: `data_game/packs/*.kpack` NEWEST-first at filesystem Initialize when the folder exists (the ONE allowed directory walk), native always last (override semantics), a corrupt pack is loud+skipped (others still mount); the `Set_FS_PriorityList` sentinel bug fixed en route (kZlib==kEndOfEnum dropped entries). The engine config gained the packs mounting through the conventional folder. TESTS (7, all OK): mixed stored/zstd/zlib roundtrip incl. block-boundary sizes (100 B / 64 KB exact / 200 KB+tail); block-range reads == full-read bytes; fallthrough+priority (unknown name silent, native wins/loses per order); corrupt magic/truncated/codec = loud+skipped, good pack still mounts; 4097-entry overflow clamped to 4096 with the cap entries readable; empty pack mounts and answers nothing; conventional-folder newest-first. VERIFIED: build RC 0, boot exit 0, 73 zircon + 245 kotek green, 5 debug_heap(996) residuals (≤6 tolerated). **B2b DONE (2026-09-12)**: the `zircon_kpacker` host tool + the offline mutation model (zircon side, `src/tools/zircon_kpacker` — the zircon_shaderpack nested-standalone-configure pattern driven by `cmake/zircon_kpacker.cmake` from the root CMakeLists, ALL-build target `zircon_kpacker_tool`). The tool COMPILES THIS MODULE'S SOURCES (`kotek_kpack_writer.cpp` + `kotek_filesystem_pack.cpp`) instead of reimplementing — writer/reader/tool can never drift on the format; the engine compile-flag surface the generated headers branch on is replicated in the tool's CMakeLists (namespace selectors, dev type, platform, std library, UNICODE trio, `/utf-8`, the log backend — every tree's `#error`/`static_assert` fires LOUDLY on a missing one, which is how the replication converged; the gfxdev tree's SPDLOG vs the main tree's CUSTOM logger shim both compile via `ZIRCON_KOTEK_LOG_LIBRARY`). CLI (exit 0 ok / 1 operational / 2 usage): `pack --root --out [--compression stored|zstd|zlib=zstd] [--order load-order.json] [--extensions .json,...] [--no-names-manifest]`, `add --pack --file --name [--compression]`, `remove --pack --name`, `list --pack`, `verify --pack`. Load order: the `--order` manifest (a json ARRAY of root-relative forward-slash paths) lists files first, unlisted files append lexicographically with a warning; default is lexicographic after the fold rule. MUTATION MODEL (owner requirement): OFFLINE wholesale rewrite — read the existing entry set (names from the NEW embedded `_kpack_names.json` sidecar entry, always written by `pack` unless `--no-names-manifest`; mutation REQUIRES it — a hash is not invertible), bytes decompressed block-by-block, the whole pack rewritten via `kpack_write_file` into a temp sibling + MoveFileEx replace; the runtime never mutates a shipped pack. `verify` is the CI integrity gate: full structural validation (magic/flags/tables/offsets/alignment/duplicate hashes) + every block decompressed cleanly (v1 stores no checksums — a stored-block byte flip is undetectable by design, documented in `--help`) + the REAL `ktkFileSystem_Pack` mounts the pack and re-reads every named entry byte-for-byte against the tool's independent block walk. `pack` sources are memory-mapped read-only (the house streaming posture — nothing whole-file in the tool's own RAM past the encoder's one-block scratch). TESTS (2 new, both OK, `kotek_core_test_filesystem.cpp`): `test_b2b_pack_mutation_rewrite_matrix_inproc` (the wholesale-rewrite model in-proc: pack stored/zstd/zlib incl. a 3-block+tail zlib entry → read back byte-identical → ADD via read-back+rewrite → the reader sees exactly the new set → REMOVE → exact set again → the verify analog: every block of every survivor decodes to the full-read span) + `test_b2b_tool_end_to_end` (the real CLI over a mixed fixture with the extensions filter: pack → list → add → remove → verify all exit 0, then the final pack mounted in-proc shows exactly the expected set with the manifest entry present; GTEST_SKIPs when no tool exe is found in the known build trees). Zircon side: `Zircon_Game.PackBootDispatcherReadsConventionalFolder` pins the BOOT dispatcher (the conventional-folder mount at Initialize + the kPack prepend; the pack-only marker json byte-exact through `Read_File`; the SAME name resolves from the PACK by default and from NATIVE under the explicit `["Native","Pack"]` config; mount→unmount→delete leaves zero residue — a fresh instance mounts nothing and the priority list stays native-first). VERIFIED: main+gfxdev Debug RC 0 (no new warnings), boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 74 zircon + 247 kotek green, 5 debug_heap(996) residuals (≤6 tolerated); the tool exercised by hand on a scratch fixture (pack/list/add/remove/verify + a corrupt zstd block detected + duplicate-add/missing-remove rejections). **B3 DONE (2026-09-12)**: the streaming API is REAL on `ktkIFileSystem`/`ktkFileSystem` (Begin_Stream/Read_Stream/Write_Stream/End_Stream + Get_DefaultStreamingBufferLength/Get_StreamingBufferLength/Get_RemainingStreamsCount/Get_TotalStreamsCount — the B0 assert-stubs replaced, the interface shape unchanged, the contract rewritten on ktkIFileSystem). Design: a dispatcher-level bounded pool of 8 `fstream_state_t` slots (`KOTEK_DEF_FILESYSTEM_FSTREAM_POOL_SIZE`) as a MEMBER of ktkFileSystem (handles are pointers into it — the documented handle-as-pointer-into-pool ABI kept, but `Get_Stream` validates pool-range+alignment+liveness before any deref so foreign handles are never dereferenced); a stream is a FORWARD-ONLY cursor (position + ONE outstanding sequential step, no seeks) with a sticky `is_failed` poison flag. Backends: native CRT (fopen + step-wise fread/fwrite at `KOTEK_DEF_FILESYSTEM_STREAM_STEP_SIZE`=4096 or the Begin override), VFM-mapped (the mapping is HELD for the stream's lifetime — map at Begin, chunked memcpy per step, unmap at End; the B1 balance counters stay exact, test-pinned), and PACK (the stream step IS the entry's 64 KB compression block via `Read_File_Block`, sequential block indices only — the path re-resolves per step so no mount pointers are held across calls; a pack stream is BYTE-IDENTICAL to the same bytes on native disk — the owner's hard requirement, test-pinned). HDD discipline: sync-sequential is this phase's floor, NO read-ahead (the one-block double-buffer — block N+1 in flight while the consumer decompresses N — is the documented B-later up-scale; IOCP/async likewise); `Write_Stream` is append-only to a FRESH native file (native only; packs stay tool-side per B2b); kReadAndWrite streams are rejected (out of scope). Semantics: Begin on a missing file = invalid handle + ONE warning (a SILENT exists-probe per backend, the all-failed line is the only one — the native Get_FileSize probe would have doubled the noise); a corrupt block mid-stream fails loudly ONCE (the pack layer's error + the poison line) and every later read errors quietly until End_Stream; End_Stream is always safe (idempotent quiet no-op on ended/foreign handles); the 9th concurrent stream = loud budget error + invalid handle and freed slots are reusable; Shutdown sweeps leaked streams loudly (fclose/unmap — no state leaks, the VFM balance survives misuse). The Begin attempt order mirrors the Read_File dispatch semantics (feature off → explicit-or-list-first; on+kAuto → the whole list; on+explicit → explicit first then the rest). TESTS (8 new `Filesystem.test_b3_*`, all OK): stream==one-shot byte parity on native small + 200,011-byte multi-chunk files (CRT and VFM shapes, counter pins, VFM map/unmap balance); the chunk-boundary matrix (1 B/4 KB/64 KB/1 MB steps, equality + counter math); 0-byte file (legal empty stream, 0 steps); write→read stream roundtrip (incl. the ustring overload + write-on-a-read-stream rejection); Begin-missing graceful + counters-0 + End idempotent; the pool 8-cap (the loud 9th, independent cursors, a full second generation of streams after reuse); pack streaming of ALL THREE codecs incl. block-boundary sizes + a 0-byte entry, pack stream == native stream byte-identical, the override chain under streaming (pack-first default, native on explicit priority); the corrupt-block case (0xFF-filled zstd block: first read fails loudly, poisoned until End, the mount NOT poisoned — the stored sibling still streams byte-exact). ZIRCON SIDE: the resource manager's kText branch does REAL IO through the API (the worker's first real IO — chunked stream into the bounded scratch, `ZIRCON_DEF_RESOURCE_MANAGER_STREAM_BUFFER_SIZE`, + Create_FromMemory; missing/empty/oversized = loud-but-graceful with is_loaded=false); the view binds AFTER the parse (Create_FromMemory replaces the json storage — a pre-parse view would dangle); `make_request`'s async contract fixed (allocate_view + req.p_resource — the async path was never reachable before: garbage pointer + invalid view id); new `get_view` accessor (the get_desc mirror). Tests: `ResourceManagerLoadTextResourceThroughWorkerStream` (async load through the REAL worker, the DOM content matches the one-shot read's bytes) + `ResourceManagerLoadTextResourceNoCache` actualized (content assertions; the fixture's `sizeof(pointer)` 7-byte-write latent bug fixed — the pre-B3 load never read the file so it never showed; the returned shared_ptr must be reset BEFORE the manager dies, its control block lives in the manager's pmr arena). VERIFIED: main + gfxdev Debug RC 0 (zero compile warnings), boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 330/330 green (74+1 zircon, 247+8 kotek), 5 debug_heap(996) residuals (≤6 tolerated), zero new boot-flow warnings. THREADING unchanged: the filesystem (streams included) is single-threaded by design this phase — per-stream state is independent, but the pool free-scan and the pack mounts' shared file handles are unsynchronized (the worker test keeps the main thread off the FS while the worker streams; the synchronization story lands with the async up-scale). Docs: `doc/git/en/configuration.md` §Tools + zircon AGENTS.md §3/Z8. **B4 DONE (2026-09-12, zircon Z23 — the FINAL phase)**: the fault-tolerant defaults chain + the embedded default blobs are ZIRCON-side per the triangle (engine policy): NEW `src/core/zircon_embedded_defaults.{h,cpp}` — four constexpr blobs (2×2 magenta/black RGBA8 checker 16 B + metadata POD; the unlit-magenta shader default as a SEMANTIC TOKEN the render side maps per backend — a compiled blob is backend-specific and backend is a runtime choice; 108 B silent PCM WAV; 2 B `{}` json) + the `zircon_embedded_default_t` POD descriptor + `get_embedded_default(type)` + the member loud-once (16-entry (type,path) dedupe, one suppression notice on overflow) + the `zircon_resolve_or_default` seam with per-type forwards; the resource manager gained desc.is_default (the editor's later badge), owns a defaults instance, lost the B3-noted native-only Is_Exists gate (pack-hosted text now loads through the dispatcher), and installs the embedded `{}` on any content failure (is_default=true + is_loaded=true, never a fault). The localization/config readers deliberately do NOT ride the json default (key echo / hardcoded defaults are the better designed floors — the Z23 row documents the choice). For kotek the phase needed ZERO changes: the dispatcher's Read_File/Begin_Stream priority walk (B0/B2/B3) IS the chain the zircon seam rides, and the B0 false+size-0 vs false+required-size contracts are exactly what the seam's missing-vs-too-small discrimination keys on. The filesystem overhaul (B0 graceful reads → B1 VFM-mapped reads → B2 .kpack reader+tool → B3 streaming → B4 defaults) is COMPLETE. VERIFIED (zircon side): main+gfxdev Debug RC 0, boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 82/82 zircon (7 new B4 tests incl. the pack-hosted-through-dispatcher and warned-capacity pins) + 255/255 kotek green, zero new boot-flow warnings, 4 debug_heap(996) residuals (≤6 tolerated). **API SIMPLICITY DONE (2026-09-12, the plan's last item — the owner's "write less code")**: free-function helpers over `ktkIFileSystem` in NEW `kotek.core.filesystem/include/kotek_filesystem_helpers.h` — HEADER-ONLY on purpose (backend-free compositions of the existing virtuals: no new virtuals, no friend access, and the PLUGIN linkage's interface purity holds — a .cpp inside the module would be a direct-call edge plugin consumers cannot link), reachable through the module umbrella `kotek_core_filesystem.h`. The five: `read_file(fs, path, buf, capacity, out_size, [priority, features])` (splits the interface's in/out size pair into explicit capacity + out-size; the B0 contracts pass through untouched — missing = false + out_size 0 + the underlying call's one warning, too-small = false + the REQUIRED size in out_size, the pointer never redirected), `read_json(fs, path, out ktkResourceText<PBS,JMS,Realloc>&)` (Get_FileSize probe → one-shot at/under `KOTEK_DEF_FILESYSTEM_STREAM_STEP_SIZE`, the B3 Begin/Read/End_Stream loop over it → Create_FromMemory; missing/empty/malformed/over-scratch/over-`_ParserBufferSize` = false + one warning, NEVER an assert — the parser-buffer check sits BEFORE Create_FromMemory so its size assert never fires on user data; the bound is the NEW named define `KOTEK_DEF_FILESYSTEM_HELPERS_JSON_SCRATCH_SIZE`=65536, sized-by-comment: the ecosystem's real json texts are ≤4 KB), `write_json(fs, path, const ktkResourceText&)` (Serialize_ToString into the scratch + one Write_File; the raw-array exemption the zircon sites carried now lives in the helper), `file_size(fs, path, out_size, [priority])` (the path-shaped Get_FileSize for call-site symmetry), `path_for(fs, eFolderIndex, name, out_path, [is_relative])` (Make_Path + `/=` in one call). The header documents the SILENCE RULE: expected-absence call sites (first-boot config, a scene without metadata, an unbuilt shader blob) keep their Is_Exists pre-check so even the one underlying warning is suppressed — the documented B0 idiom, not a workaround. Tests: 5 NEW `Filesystem.test_helpers_*` suites in kotek_core_test_filesystem.cpp (read_file happy/too-small-then-retry-with-the-reported-size/missing + the file_size contract; read_json present/missing/malformed/empty/over-`_ParserBufferSize` — the last would hit Create_FromMemory's assert without the graceful pre-check; write_json→read_json roundtrip through the real file incl. the string value via the backend-portable find/as_string subset; path_for byte-equal to the Make_Path two-step; a 6000-byte json (> the stream step) through the helper's streaming branch DOM-identical to the one-shot+Create_FromMemory reference with the content pinned). En-route test fix: `small` is an rpcndr.h macro on Windows — the too-small fixture is `small_buf`. VERIFIED: main+gfxdev Debug RC 0 (the 4 C4273 dll-linkage warnings on the game-module entries are pre-existing full-PCH-rebuild noise, untouched files), boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 82/82 zircon + 260/260 kotek (255+5) green with the 5 new OK lines, zero new boot-flow warnings (every helpers-* warning line is a test's intentional contract proof), 4 debug_heap(996) residuals (≤6 tolerated), vfm balance 9/9. Zircon side (Z8): the call-site sweep — zircon_config serialize/deserialize, zircon_scene_metadata save/load, the three bgfx shader-blob readers + model_static's mesh path, gltf_read_file. **EXAMPLES + HDD BENCHMARK (2026-09-12, owner-requested follow-up)**: the consumer usage guide `doc/git/en/filesystem.md` (zircon repo; linked from the root README + configuration.md §Tools) — five-minute helpers, the override-chain diagram (pack-first default, `["Native","Pack"]` flip), the streaming contract (forward-only, the pack step = the 64 KB compression block, byte-identical guarantee), zircon_kpacker end-to-end, the embedded-defaults chain (`is_default` + the loud-once log), and the four complex scenarios (texture/mesh streaming with a bounded scratch, cached small-hot files, never-materialize big cold regions). Three NEW `Filesystem.test_hdd_*` suites in kotek_core_test_filesystem.cpp (default tier, ~1.5 s total, fixtures self-clean under data_user/tests): `test_hdd_stream_sequential_vs_scattered` (a 64 MB deterministic fixture: one-shot vs 4 KB-step vs 64 KB-step forward streams vs 256 scattered 4 KB seek-reads through std::ifstream; byte-equality of (a)/(b)/(c) asserted; times LOGGED ONLY as `[hdd-bench]` KOTEK_MESSAGE rows; the DESIGN INVARIANT pinned instead of times — the remaining-count strictly decreases by exactly 1 per read and the counted reads equal ceil(size/step): 16384 at 4 KB, 1024 at 64 KB), `test_hdd_pack_stream_block_discipline` (a 4 MB zstd entry: the pack stream byte-identical to the native reference per block, exactly 64 block reads (Get_TotalStreamsCount + the loop counter), every read exactly ONE 64 KB block into ONE reused 64 KB scratch — the no-whole-file-inflation memory-shape proof; pack-zstd vs native MB/s logged), `test_hdd_cached_vs_streamed_shape` (the guide's §6c/§6d in code: a ~2 KB config re-read 200× one-shot (the OS-page-cache path, total time logged) vs a 64 MB region streamed once through ONE 64 KB scratch — the peak-extra-memory-is-the-step shape asserted via Get_StreamingBufferLength + per-read `got <= step`, 1024× smaller than the file). Suite comments carry the Debug-CRT/page-cache-warm caveat: numbers are evidence, not gates. Measured on the owner's machine (Debug, page-cache warm): one-shot 64 MB 29.4 ms/2177 MB/s; 4 KB-step stream 98.3 ms/651 MB/s (16384 reads); 64 KB-step stream 22.1 ms/2900 MB/s (1024 reads); scattered 256×4 KB 6.9 ms/146 MB/s; pack-zstd 4 MB 17.3 ms/231 MB/s vs native 7.9 ms/507 MB/s (64 block reads each); 200 config rereads 32.4 ms; 64 MB region streamed 211.6 ms/302 MB/s at a 65536-byte peak. VERIFIED: main+gfxdev Debug RC 0 (no new warnings), boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 82/82 zircon + 263/263 kotek green (the 3 new OK lines), 0 debug_heap(996) residuals, fixtures self-cleaned, git status = intended files only |
