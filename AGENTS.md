...
> Last updated: 2026-07-25 (CI Release-static killers fixed: vector constexpr
> LNK2019 + tracy manual-lifetime portfile patch; K11 phase 1 NRI live — see the
> K5/K11 registry rows).
...
| K4 | Per-module backend config matrix: {no-deps+kotek impl} / {deps backends} / {user impl}; add missing `KOTEK_USE_*_USER_IMPL`-style guards where absent | in-progress (2026-07-25) | matrix design in §1a; user-impl mechanism = K21 (§5a). VALIDATION: math is the first fully-covered module — all three backends compile-validated again today post-NRI/Z16 (GLM = default in every build; DXM and KOTEK_OWN via `-DKOTEK_MATH_LIBRARY=DXM|KOTEK_OWN`, 0 errors each, parity suite `kotek_core_test_math_backends.cpp` compiles in both). CI: `matrix.yml` gained `math-dxm` + `math-own` legs (it only had math-glm), fixed the latent `--config Debug` for every leg (minimal-release silently built Debug; now `matrix.cfg` per leg), cache key aligned to the v2+hashFiles scheme of the other workflows, timeout 120→180 (cold-cache dep rebuilds). REMAINING: per-module guard uniformity sweep + CI legs for containers (std/boost/ETL), threading (std/TBB), logging (spdlog/own), json (boost/own), window (GLFW), UI (imgui/rmlui/cef), ECS (pico/entt), video (dav1d), memory (mimalloc/std) — the same {deps ∪ no-deps-own} pattern math now has |
| K5 | GitHub CI/CD: compile EACH module isolated (all supported backends; skip user-impl) + whole solution; free plan; disk-space strategy (vcpkg binary cache / ccache; public repo ⇒ unlimited minutes, cache limit 10 GB) | in-progress (2026-07-23) | workflows exist (build/tests/modules on every push); fixed: vcpkg cache 6.5GB→1.2GB (quota-safe, key hashed on dep list), tests.yml now build-only (runtime gtests need game.ktk → zircon CI), vcpkg pinned to the validated snapshot 1bf99941 (was floating master — likely configure-failure cause). UPDATE 2026-07-25: deps+configure green on all legs (mimalloc added to the minimal dep set, pwsh wildcard in dep-log names sanitized, per-port retry with `deps-<port>.log`, timeouts 90→180); build failures fixed since: gtest body guard in the unordered_set test, LNK2038 CRT include order (`compiler_globals.cmake` after `project()`), Tracy `FrameMark` collision (`tracy::Profiler::SendFrameMark`), `contents: write` + clone-based `ci-logs` publishing in all three workflows (the public read path for CI failures: `ci-logs/<run-id>/<leg>/build-output.log` — the jobs API needs admin, this does not), `badges` job made first-push safe. LAST KNOWN RED (run 30120021260 on 7021a6e): `kotek_std_math_matrix4x4_f.cpp:921` C3867 — 7021a6e dropped `constexpr` from the header declarations of `size_of/get_column_count/get_row_count` but the commit missed the matching .cpp definitions, MSVC reads the mismatch as a member-pointer reference; fixed in the K11-phase-1 commit (build/tests/modules were green before that regression). RELEASE-STATIC RED (run 30156175901 on 8ced470; Debug-static/tests/modules green): two LNK2019 classes at the kotek.exe link — (1) `vector1f-4f::size_of/get_column_count/get_row_count`: declared+defined `constexpr`, MSVC /O2 inlines them in their own TUs and discards the out-of-line bodies while `matrixnf_view.obj` still references the symbols — fixed by dropping `constexpr` on all 24 spots (the matrix4x4f precedent; constexpr was buying nothing and breaking the link model); (2) `tracy::StartupProfiler/ShutdownProfiler`: ktkProfiler drives manual lifetime (`TRACY_DELAYED_INIT`+`TRACY_MANUAL_LIFETIME` from defines.static.profilers, ON in Release by default) but the vcpkg tracy port never forwards those cmake options, so stock `TracyClient.lib` lacks the entry points — fixed by an idempotent portfile patch in `cmake/windows/vcpkg_nuget_minimal.cmake` (forwards both options; one-time remove+install rebuild tracked by `vcpkg/installed/.kotek_tracy_manual_lifetime` — plain install/--recurse do NOT rebuild same-version packages, verified). Both masked locally by incremental caches, both reproduced from scratch (`build-ci-repro`: `cmake -S . -B build-ci-repro -DCMAKE_BUILD_TYPE=Release`), both verified: from-scratch Release of kotek.exe links green. CONFIRMED ON CI (run 30158398077 on 8670cd7): **build workflow all four legs green** (Release-static + Debug-static required legs pass; the two dynamic legs stay continue-on-error by K18 design and report success as such). TESTS.RED the same run (30158398086): configure failed 3× on the cache-busting cold tree — 8670cd7 edited `vcpkg_nuget_minimal.cmake` (the cache key file), tests.yml has no deps steps so the whole from-source dep set had to build inside its configure and flaked across attempts; the diag re-run then passed on the resumed tree, hiding the original output. Fixes pushed in bd6d211: `restore-keys` fallback in ALL four workflows (key bump → previous generation's tree restored, only changed ports rebuild) + tests.yml tees real `configure-attempt<N>.log`/`build-output.log` into the ci-logs publish (the publish previously captured only the diag re-run). Owner 2026-07-23: CI must build THREE linkage configurations — full static (all .lib + kotek.exe), full dynamic (all .dll + kotek.exe; **blocked today by the cyclic module graph, K18 — CI leg = continue-on-error**), default (kotek static + game.ktk) — zircon Z13 tracks the same |
| K6 | CMake command printing all kotek options + meaning (help) | done (2026-07-25) | `cmake -DKOTEK_HELP=ON` — prints the curated registry in `cmake/options_help.cmake` (name|default|description, 34 entries) and stops before any dep work (fast, no vcpkg fetch). Registry made current: KOTEK_NRI, the three STATIC-dev-type game-module names, per-config profiler defaults (OFF Debug/ON Release), MIMALLOC default + leak-detection interplay, NRI-live renderer notes; the file header carries the rule that every new option joins the registry in the same commit. Two `;`-in-description bugs fixed (semicolons are CMake list separators and split entries). Verified: 0 errors, 34 options print |
