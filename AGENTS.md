...
> Last updated: 2026-07-31 (logging-CUSTOM own backend landed + two systemic
> fixes: generated-preprocessors header is reset per configure — backend flips
> in an existing tree were silently shadowed by stale appended defines; logger
> file sink moved to a Win32 OS handle — a CRT FILE* crossing the exe/game.ktk
> boundary made `all.log` silently empty in SHARED dev type; see K4 + §8).
