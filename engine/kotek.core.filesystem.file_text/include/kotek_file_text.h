...
public:
	...
	*/
	...
	{
		this->m_json.Write<ktk::json::array>(field_name, {data.get_as_is()});
	}
		...
		const ktk::string& field_name, const char8_t* p_string) noexcept
	{
		...
		this->m_json.Write<ktk::json::array>(field_name, {str.get_as_is()});
	}
	 ...
	 */
	template <>
	void Write<const char16_t*>(
		const ktk::string& field_name, const char16_t* p_string) noexcept
	{
		ktk::string str;
		str = p_string;
	 ...
	 */
	template <>
	void Write<const char32_t*>(
		const ktk::string& field_name, const char32_t* p_string) noexcept
	{
		ktk::string str;
		str = p_string;
