...
/* TODO: перенести тесты в отдельное приложение, остальное тестировать динамически
		BOOST_AUTO_TEST_CASE(load_shader_by_invalid_name)
		{
		...
		BOOST_AUTO_TEST_CASE(load_shader_by_valid_name)
		{
			...
			auto render_device = std::make_shared<vk::kotek_render_device>();
		...
		BOOST_AUTO_TEST_CASE(load_shader_from_string_invalid)
		{
			...
			ktk::string_legacy code =
				...
				"   outColor = inColor;\n"
				"   outTexCoord = inTexCoord;\n"
				"   gl_Position123 = myVertexBuffer.ProjectionMatrix * pos;\n"
				"}\n";
