...
static bool SearchSortedUint32(
	const uint32_t* arr, size_t size, uint32_t target)
{
	size_t lo = 0;
	size_t hi = size;
	while (lo < hi)
...
static SpvReflectResult IntersectSortedUint32(const uint32_t* p_arr0,
	size_t arr0_size, const uint32_t* p_arr1, size_t arr1_size,
...
static SpvReflectResult ReadU32(
	SpvReflectPrvParser* p_parser, uint32_t word_offset, uint32_t* p_value)
{
...
static SpvReflectResult ReadStr(SpvReflectPrvParser* p_parser,
	uint32_t word_offset, uint32_t word_index, uint32_t word_count,
...
static SpvReflectPrvNode* FindNode(
	SpvReflectPrvParser* p_parser, uint32_t result_id)
{
...
static SpvReflectTypeDescription* FindType(
	SpvReflectShaderModule* p_module, uint32_t type_id)
{
...
static SpvReflectResult CreateParser(
	size_t size, void* p_code, SpvReflectPrvParser* p_parser)
{
	if (p_code == NULL)
	{
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser* p_parser)
{
	...
	p_parser->nodes = (SpvReflectPrvNode*)calloc(
		p_parser->node_count, sizeof(*(p_parser->nodes)));
	if (IsNull(p_parser->nodes))
	{
		return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
	}
	...
	while (spirv_word_index < p_parser->spirv_word_count)
	{
		...
		case SpvOpSource:
		{
			CHECKED_READU32_CAST(p_parser, p_node->word_offset + 1,
				SpvSourceLanguage, p_parser->source_language);
		...
		{
			CHECKED_READU32(
				p_parser, p_node->word_offset + 1, p_node->result_id);
			p_node->is_type = true;
		}
		break;
		...
		case SpvOpTypeImage:
		{
			...
			CHECKED_READU32(
				p_parser, p_node->word_offset + 3, p_node->image_traits.dim);
			CHECKED_READU32(
				p_parser, p_node->word_offset + 4, p_node->image_traits.depth);
			CHECKED_READU32(p_parser, p_node->word_offset + 5,
				p_node->image_traits.arrayed);
			CHECKED_READU32(
				p_parser, p_node->word_offset + 6, p_node->image_traits.ms);
		...
		case SpvOpTypeSampledImage:
		{
			CHECKED_READU32(
				p_parser, p_node->word_offset + 1, p_node->result_id);
			CHECKED_READU32(
				p_parser, p_node->word_offset + 2, p_node->image_type_id);
			p_node->is_type = true;
		}
		break;
		...
		case SpvOpTypePointer:
		{
			CHECKED_READU32(
				p_parser, p_node->word_offset + 1, p_node->result_id);
			CHECKED_READU32(
				p_parser, p_node->word_offset + 2, p_node->storage_class);
		...
		{
			CHECKED_READU32(
				p_parser, p_node->word_offset + 1, p_node->result_type_id);
			CHECKED_READU32(
				p_parser, p_node->word_offset + 2, p_node->result_id);
		}
		break;
		...
		case SpvOpVariable:
		{
			...
			CHECKED_READU32(
				p_parser, p_node->word_offset + 2, p_node->result_id);
			CHECKED_READU32(
				p_parser, p_node->word_offset + 3, p_node->storage_class);
		}
		break;
		...
		case SpvOpAccessChain:
		{
			...
			CHECKED_READU32(
				p_parser, p_node->word_offset + 2, p_access_chain->result_id);
			CHECKED_READU32(
				p_parser, p_node->word_offset + 3, p_access_chain->base_id);
			//
			...
			if (p_access_chain->index_count > 0)
			{
					 ...
					 index_index < p_access_chain->index_count; ++index_index)
				{
						...
						(p_index_value_node->op == SpvOpConstant))
					{
						...
						CHECKED_READU32(p_parser,
							p_index_value_node->word_offset + 3, index_value);
...
static SpvReflectResult ParseStrings(SpvReflectPrvParser* p_parser)
{
		...
		IsNotNull(p_parser->nodes))
	{
		...
		for (size_t i = 0; i < p_parser->node_count; ++i)
		{
			...
			CHECKED_READU32(
				p_parser, p_node->word_offset + 1, p_string->result_id);
...
static SpvReflectResult ParseSource(
...
static SpvReflectResult ParseFunction(SpvReflectPrvParser* p_parser,
	SpvReflectPrvNode* p_func_node, SpvReflectPrvFunction* p_func,
	size_t first_label_index)
{
	...
	{
		p_func->accessed_ptrs = (uint32_t*)calloc(
			p_func->accessed_ptr_count, sizeof(*(p_func->accessed_ptrs)));
	...
	p_func->accessed_ptr_count = (uint32_t)DedupSortedUint32(
		p_func->accessed_ptrs, p_func->accessed_ptr_count);
		...
		IsNotNull(p_parser->nodes))
	{
		for (size_t i = 0; i < p_parser->node_count; ++i)
		{
			...
			if (p_node->op == SpvOpMemberName)
			{
				...
				CHECKED_READU32(
					p_parser, p_node->word_offset + 2, member_index);
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser* p_parser)
{
	for (uint32_t i = 0; i < p_parser->node_count; ++i)
	{
		...
		CHECKED_READU32(
			p_parser, p_node->word_offset + member_offset + 2, decoration);
		...
		case SpvDecorationArrayStride:
		{
			...
			CHECKED_READU32(
				p_parser, word_offset, p_target_decorations->array_stride);
		}
		break;
		...
		case SpvDecorationMatrixStride:
		{
			uint32_t word_offset = p_node->word_offset + member_offset + 3;
			CHECKED_READU32(
				p_parser, word_offset, p_target_decorations->matrix_stride);
		}
		break;
		...
		case SpvDecorationLocation:
		{
			uint32_t word_offset = p_node->word_offset + member_offset + 3;
			CHECKED_READU32(
				p_parser, word_offset, p_target_decorations->location.value);
		...
		case SpvDecorationBinding:
		{
			uint32_t word_offset = p_node->word_offset + member_offset + 3;
			CHECKED_READU32(
				p_parser, word_offset, p_target_decorations->binding.value);
		...
		case SpvDecorationDescriptorSet:
		{
			uint32_t word_offset = p_node->word_offset + member_offset + 3;
			CHECKED_READU32(
				p_parser, word_offset, p_target_decorations->set.value);
		...
		case SpvDecorationOffset:
		{
			uint32_t word_offset = p_node->word_offset + member_offset + 3;
			CHECKED_READU32(
				p_parser, word_offset, p_target_decorations->offset.value);
	...
	SpvReflectShaderModule* p_module, SpvReflectTypeDescription* p_type)
{
	...
	if (result == SPV_REFLECT_RESULT_SUCCESS)
	{
		...
		case SpvOpTypeVector:
		{
			...
			IF_READU32(
				result, p_parser, p_node->word_offset + 2, component_type_id);
			IF_READU32(result, p_parser, p_node->word_offset + 3,
		...
		case SpvOpTypeMatrix:
		{
			...
			IF_READU32(
				result, p_parser, p_node->word_offset + 2, column_type_id);
			IF_READU32(result, p_parser, p_node->word_offset + 3,
		...
		case SpvOpTypeImage:
		{
			...
			IF_READU32(
				result, p_parser, p_node->word_offset + 2, sampled_type_id);
			SpvReflectPrvNode* p_next_node =
			...
			IF_READU32_CAST(result, p_parser, p_node->word_offset + 8,
				SpvImageFormat, p_type->traits.image.image_format);
		}
		break;
		...
		case SpvOpTypeSampledImage:
		{
			...
			IF_READU32(
				result, p_parser, p_node->word_offset + 2, image_type_id);
		...
		case SpvOpTypeArray:
		{
			...
			if (result == SPV_REFLECT_RESULT_SUCCESS)
			{
				...
				IF_READU32(
					result, p_parser, p_node->word_offset + 2, element_type_id);
				IF_READU32(
					result, p_parser, p_node->word_offset + 3, length_id);
				...
				if (IsNotNull(p_length_node))
				{
					...
					{
						p_type->traits.array
							.dims[p_type->traits.array.dims_count] = 0xFFFFFFFF;
					...
					else
					{
						...
						if (result == SPV_REFLECT_RESULT_SUCCESS)
						{
							...
							// offset
							p_type->traits.array
								.dims[p_type->traits.array.dims_count] = length;
							p_type->traits.array.dims_count += 1;
						}
						else
						{
					...
					{
						result = ParseType(
							p_parser, p_next_node, NULL, p_module, p_type);
					}
				}
				else
				{
		...
		case SpvOpTypeStruct:
		{
				 ...
				 ++word_index, ++member_index)
			{
				...
				result = ParseType(p_parser, p_member_node,
					p_member_decorations, p_module, p_member_type);
				if (result != SPV_REFLECT_RESULT_SUCCESS)
				{
					break;
				}
	...
	SpvReflectBlockVariable* p_var)
{
		 ...
		 ++member_index)
	{
		...
		case SpvOpTypeArray:
		{
			...
			{
				SpvReflectResult result =
					ParseDescriptorBlockVariableSizes(p_parser, p_module, false,
						true, is_parent_rta, p_member_var);
				if (result != SPV_REFLECT_RESULT_SUCCESS)
				{
					return result;
				}
		...
		case SpvOpTypeStruct:
		{
			SpvReflectResult result =
				ParseDescriptorBlockVariableSizes(p_parser, p_module, false,
					is_parent_aos, is_parent_rta, p_member_var);
			if (result != SPV_REFLECT_RESULT_SUCCESS)
			{
				return result;
			}
	...
	SpvOp override_op_type, SpvReflectBlockVariable* p_var)
{
	...
	case SpvOpTypeArray:
	{
		...
		if (index_index < p_access_chain->index_count)
		{
			...
			// index
			SpvReflectResult result =
				ParseDescriptorBlockVariableUsage(p_parser, p_module,
	...
	SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
		 ...
		 ++descriptor_index)
	{
			 ...
			 ++access_chain_index)
		{
			...
			result = ParseDescriptorBlockVariableUsage(p_parser, p_module,
				p_access_chain, 0, (SpvOp)INVALID_VALUE, &p_descriptor->block);
			if (result != SPV_REFLECT_RESULT_SUCCESS)
			{
				return result;
			}
		...
		result = ParseDescriptorBlockVariableSizes(p_parser, p_module, true,
			false, is_parent_rta, &p_descriptor->block);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		}
...
static SpvReflectResult ParseFormat(
	const SpvReflectTypeDescription* p_type, SpvReflectFormat* p_format)
{
	...
	if (p_type->member_count > 0)
	{
			 ...
			 member_index < p_type_node->member_count; ++member_index)
		{
			...
			SpvReflectInterfaceVariable* p_member_var =
				&p_var->members[member_index];
			SpvReflectResult result =
				ParseInterfaceVariable(p_parser, p_member_decorations, p_module,
...
static SpvReflectResult TraverseCallGraph(SpvReflectPrvParser* p_parser,
	SpvReflectPrvFunction* p_func, size_t* p_func_count, uint32_t* p_func_ids,
	uint32_t depth)
{
	...
	for (size_t i = 0; i < p_func->callee_count; ++i)
	{
		SpvReflectResult result = TraverseCallGraph(p_parser,
			p_func->callee_ptrs[i], p_func_count, p_func_ids, depth + 1);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		}
	...
	size_t push_constant_count, uint32_t* push_constants)
{
	...
	{
		p_called_functions = (uint32_t*)calloc(
			called_function_count, sizeof(*p_called_functions));
	...
	result = TraverseCallGraph(
		p_parser, p_func, &called_function_count, p_called_functions, 0);
	if (result != SPV_REFLECT_RESULT_SUCCESS)
	{
		return result;
	}
	...
	{
		used_variables =
			(uint32_t*)calloc(used_variable_count, sizeof(*used_variables));
	...
	SpvReflectResult result0 =
		IntersectSortedUint32(used_variables, used_variable_count, uniforms,
	...
	SpvReflectResult result1 = IntersectSortedUint32(used_variables,
		used_variable_count, push_constants, push_constant_count,
	...
	for (uint32_t j = 0; j < p_module->descriptor_binding_count; ++j)
	{
		...
		bool found = SearchSortedUint32(
			used_variables, used_variable_count, p_binding->spirv_id);
		if (found)
		{
	...
	SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
	...
	if ((result = EnumerateAllPushConstants(p_module, &push_constant_count,
			 &push_constants)) != SPV_REFLECT_RESULT_SUCCESS)
	{
		return result;
	}
		 ...
		 ++i)
	{
		...
		CHECKED_READU32_CAST(p_parser, p_node->word_offset + 1,
			SpvExecutionModel, p_entry_point->spirv_execution_model);
		...
		result = ReadStr(p_parser, p_node->word_offset + name_start_word_offset,
			0, p_node->word_count, &name_length_with_terminator, NULL);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		}
		p_entry_point->name = (const char*)(p_parser->spirv_code +
			 ...
			 ++var_index)
		{
			...
			CHECKED_READU32(
				p_parser, p_node->word_offset + offset, var_result_id);
		...
		result = ParseStaticallyUsedResources(p_parser, p_module, p_entry_point,
			uniform_count, uniforms, push_constant_count, push_constants);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		}
	...
	SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
	...
	p_module->push_constant_blocks =
		(SpvReflectBlockVariable*)calloc(p_module->push_constant_block_count,
	...
	for (uint32_t i = 0; i < p_module->entry_point_count; ++i)
	{
		...
		for (uint32_t j = 0; j < p_module->descriptor_set_count; ++j)
		{
			...
			for (uint32_t k = 0; k < p_set->binding_count; ++k)
			{
				bool found = SearchSortedUint32(p_entry->used_uniforms,
					p_entry->used_uniform_count, p_set->bindings[k]->spirv_id);
				if (found)
				{
		...
		if (p_entry->descriptor_set_count > 0)
		{
			p_entry->descriptor_sets =
				(SpvReflectDescriptorSet*)calloc(p_entry->descriptor_set_count,
	...
	if (IsNotNull(p_module))
	{
			 ...
			 ++index)
		{
			const SpvReflectDescriptorBinding* p_potential =
				&p_module->descriptor_bindings[index];
			bool found = SearchSortedUint32(p_entry->used_uniforms,
				p_entry->used_uniform_count, p_potential->spirv_id);
			if ((p_potential->binding == binding_number) &&
	...
	uint32_t new_binding_number, uint32_t optional_new_set_number)
{
	return spvReflectChangeDescriptorBindingNumbers(p_module,
		p_descriptor_binding, new_binding_number, optional_new_set_number);
}
