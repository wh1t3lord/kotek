...
ktk::string ktkJson::GetString(const ktk::string& key_name) const noexcept
{
	ktk::string result;
	...
	if (is_array)
	{
		ktk::json::array buffer = this->m_json.at(key_name_legacy).as_array();
		...
		for (const auto& value : buffer)
		{
			...
			ktk::tchar symbol = static_cast<ktk::tchar>(value.as_int64());
	...
	{
		const bool is_string = this->m_json.at(key_name_legacy).is_string();
