	...
	m_p_manager_resource{}, m_p_main_manager{p_main_manager}
{
	...
	this->m_p_render_resource_manager = dynamic_cast<ktkRenderResourceManager*>(
	...
	gl::eRenderGraphBuilderPipelineRenderingType type_videocard_pipeline)
{
	...
	this->m_p_manager_resource = this->m_p_main_manager->GetResourceManager();
		...
		info_create)
{
	...
	ktk::size_t memory_for_allocation_in_bytes = ktk::align_down<ktk::size_t>(
	...
	KOTEK_ASSERT(
		map_buffer_name_and_buffer_module.find(info_buffer.Get_BufferName()) ==
	...
	map_buffer_name_and_buffer_module[info_buffer.Get_BufferName()] =
		this->m_p_render_resource_manager->Get_ManagerShader()->Create_Buffer(
			memory_for_allocation_in_bytes, info_buffer.Get_BufferObject(),
			info_buffer.Get_Usage(), info_buffer);
	...
	KOTEK_MESSAGE("create buffer [{}] with size {} Kb ({} Mb) for render "
		...
		reinterpret_cast<const char*>(info_create.Get_RenderPassName().c_str()),
		reinterpret_cast<const char*>(info_buffer.Get_ShaderName().c_str()));
#endif
}
...
ktkRenderGraphSimplifiedResourceManager::Get_Storage_Buffers_By_RenderPassName(
	const ktk::ustring& render_pass_name) const KOTEK_CPP_KEYWORD_NOEXCEPT
{
	...
	{
#ifdef KOTEK_DEBUG
		KOTEK_MESSAGE_WARNING("render_pass_name [{}] doesn't exist for buffers",
			render_pass_name);
#endif
		return p_result;
	}

	p_result = &this->m_render_passes_and_their_buffers.at(render_pass_name);
...
ktkRenderGraphSimplifiedResourceManager::Get_Storage_Buffers(
	void) const KOTEK_CPP_KEYWORD_NOEXCEPT
{
	return &this->m_render_passes_and_their_buffers;
}
		...
		shaders_for_current_render_pass)
{
		...
		shaders_for_current_render_pass)
	{
		...
		for (const auto& [shader_name, map_shaders] :
			map_shader_type_info_creation)
		{
			for (const auto& [shader_type, info_shader_creation] : map_shaders)
			{
#ifdef KOTEK_DEBUG
				if (this->m_render_passes_and_its_shaders.find(pipeline_name) !=
					this->m_render_passes_and_its_shaders.end())
				{
					if (this->m_render_passes_and_its_shaders.at(pipeline_name)
							.find(shader_name) !=
						this->m_render_passes_and_its_shaders.at(pipeline_name)
							.end())
					{
						KOTEK_ASSERT(this->m_render_passes_and_its_shaders
										 .at(pipeline_name)
										 .at(shader_name)
										 .find(shader_type) ==
								this->m_render_passes_and_its_shaders
									...
									.at(shader_name)
									.end(),
							"duplicate is found. It means that you "
							"registered two shader type with the same name "
							"like two vertex shaders with the same shader "
							"name!");
					}
				}
#endif
...
void ktkRenderGraphSimplifiedResourceManager::Destroy_Shaders(void)
{
		...
		this->m_render_passes_and_its_shaders)
	{
		...
		{
			for (const auto& [shader_type, shader_module] : map_shaders)
			{
#ifdef KOTEK_DEBUG
				...
				map_shader_type_shader_module)
			{
				for (const auto& [shader_type, shader_module] : map_shaders)
				{
					...
					KOTEK_MESSAGE("attached shader type to "
								  "render_pass_name[{}] shader_name[{}]: [{}]",
						reinterpret_cast<const char*>(render_pass_name.c_str()),
						reinterpret_cast<const char*>(shader_name.c_str()),
						gl::helper::Translate_ShaderType(shader_type));
#endif
