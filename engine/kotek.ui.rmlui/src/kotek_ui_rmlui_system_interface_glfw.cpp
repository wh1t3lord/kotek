	...
	Rml::Context* context, int key, int action, int mods)
{
	...
	case GLFW_REPEAT:
		result = context->ProcessKeyDown(
			RmlGLFW::ConvertKey(key), RmlGLFW::ConvertKeyModifiers(mods));
	...
	Rml::Context* context, double xpos, double ypos, int mods)
{
	...
	bool result = context->ProcessMouseMove(
		int(xpos), int(ypos), RmlGLFW::ConvertKeyModifiers(mods));
	return result;
}

bool RmlGLFW::ProcessMouseButtonCallback(
	Rml::Context* context, int button, int action, int mods)
{
	...
	case GLFW_PRESS:
		result = context->ProcessMouseButtonDown(
			button, RmlGLFW::ConvertKeyModifiers(mods));
		break;
	case GLFW_RELEASE:
...
bool RmlGLFW::ProcessScrollCallback(
	Rml::Context* context, double yoffset, int mods)
{
	if (!context)
		return true;

	bool result = context->ProcessMouseWheel(
		-float(yoffset), RmlGLFW::ConvertKeyModifiers(mods));
	return result;
}

void RmlGLFW::ProcessFramebufferSizeCallback(
