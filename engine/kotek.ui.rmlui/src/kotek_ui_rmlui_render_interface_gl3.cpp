...
namespace Gfx
{
	...
	static const char* const program_uniform_names[(
		size_t)ProgramUniform::Count] = {"_translate", "_transform", "_tex"};
	...
	static const char* const
		vertex_attribute_names[(size_t)VertexAttribute::Count] = {
	...
	static void CheckGLError(const char* operation_name)
	{
		...
		if (error_code != GL_NO_ERROR)
		{
			...
			Rml::Log::Message(Rml::Log::LT_ERROR,
				"OpenGL error during %s. Error code 0x%x (%s).", operation_name,
	...
	static bool CreateProgram(
		GLuint vertex_shader, GLuint fragment_shader, ProgramData& out_program)
	{
		...
		if (status == GL_FALSE)
		{
			...
			Rml::Log::Message(Rml::Log::LT_ERROR,
				"OpenGL program linking failure: %s", info_log_string);
			delete[] info_log_string;
			glDeleteProgram(id);
			return false;
		}
		...
		for (int unif = 0; unif < num_active_uniforms; ++unif)
		{
			...
			glGetActiveUniform(id, unif, name_size, &actual_length, &array_size,
				&type, name_buf);
			...
			{
				out_program.uniform_locations[(size_t)program_uniform] =
					location;
			}
			else
			{
				Rml::Log::Message(Rml::Log::LT_ERROR,
					"OpenGL program uses unknown uniform '%s'.", name_buf);
				return false;
			}
		}
	...
	Rml::TextureHandle texture)
{
	...
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Position, 2, GL_FLOAT,
	...
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Color0, 4,
	...
	glVertexAttribPointer((GLuint)Gfx::VertexAttribute::TexCoord0, 2, GL_FLOAT,
		GL_FALSE, sizeof(Rml::Vertex),
	...
	Rml::CompiledGeometryHandle handle, const Rml::Vector2f& translation)
{
	...
	if (geometry->texture)
	{
		...
		SubmitTransformUniform(ProgramId::Texture,
			shaders->program_texture
				.uniform_locations[(size_t)Gfx::ProgramUniform::Transform]);
		glUniform2fv(
			shaders->program_texture
				.uniform_locations[(size_t)Gfx::ProgramUniform::Translate],
			1, &translation.x);
	}
	else
	{
	...
	glDrawElements(
		GL_TRIANGLES, geometry->draw_count, GL_UNSIGNED_INT, (const GLvoid*)0);
...
void ktkRenderInterface_GL3::SetTransform(const Rml::Matrix4f* new_transform)
{
	...
	transform = projection *
		(new_transform ? *new_transform : Rml::Matrix4f::Identity());
	...
	ProgramId program_id, int uniform_location)
{
	if ((int)program_id & (int)transform_dirty_state)
	{
		...
		transform_dirty_state =
			ProgramId((int)transform_dirty_state & ~(int)program_id);
	}
}
