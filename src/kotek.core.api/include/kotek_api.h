...
/// \~english @brief This class
/// stands for implementing own
/// loader for that case it has
/// only 'Load' method names it
/// means that you specify what
/// the implementation does.
/// Does it load text or model
/// or even any other type? Our
/// standard is that user
/// implements one general class
/// for each type of possible
/// data Like model, text,
/// video, but he provides and
/// implements a classes for
/// each format and user must
/// follow to these rules For
/// example, I want to have a
/// loader for Videos, I create
/// ktkLoaderVideo (formally
/// ktkLoaderTypeOfDataName)
/// Then, I create for each
/// format of that data type
/// like ktkLoaderVideo_MP4,
/// ktkLoaderVideo_AV1,
/// ktkLoaderVideo_XXX and etc
/// In general class it
/// registers all classes what
/// we created earlier and we
/// register to main
/// ktkResourceManagerLoader You
/// need to provide two private
/// methods (they are not
/// virtual!!!, you provide them
/// by your own), like say it is
/// Register_Loaders and
/// Delete_Loaders (see our
/// default implementation
/// loader classes) and with a
/// such design you will
/// understand where you have
/// 'general' class and where
/// you have the ended
/// implementation of a format,
/// because ended implementation
/// doesn't have any registered
/// loaders because it is final!
///
/// So let's see on detailed
/// illustration
// clang-format off
/*
...
public:
	...
	/// on stack. It means it
	/// doesn't create
	/// shared_ptr and uses only
	/// for situations where
	/// user wants to pass an
	/// instance that was
	/// allocated temporary
	/// @param path
	/// @param
	/// object_from_construct
	/// @return
	virtual bool Load(
		const ktk_filesystem_path& path,
		kun_core ktkResourceHandle object_from_construct
	) noexcept = 0;

	virtual bool
	DetectTypeByFullPath(const ktk_filesystem_path& path
	) noexcept = 0;

	/// \~english @brief This is
	/// only for user. You just
	...
	/// \~english @brief This
	/// method is for validation
	/// purpose and for
	/// automatization purpose
	/// when LoaderManager picks
	/// the loader for analyzed
	/// path with extension. So
	/// for example you pass the
	/// path like
	/// 'C:/mygame/gamedata/models/some_stuff/test.obj'
	/// so if you registered a
	/// loader that contains
	/// format '.obj' it must
	/// return
	/// eResourceLoadingType::kModel
	/// enum value.
	/// @return What you
	/// specified in overriding,
	/// otherwise default value
	/// is
	/// eResourceLoadingType::kUnknown.
	/// If you got
	/// eResourceLoadingType::kUnknown
	...
	// TODO: ask a question to
	// community: is it
	// reasonable to support
	// files without formats?

	/// \~english @brief this
	/// method needs in case
	/// when you have one loader
	/// (for example you
	/// implement for
	/// eResourceLoadingType::kModel)
	/// but it specifies three
	/// different formats like
	/// (.fbx, .obj and .glb) in
	/// such case you need to
	/// implement three own
	/// loaders (because the
	/// implementation of
	/// loading process is
	/// different), but you need
	/// to pass them to this
	/// class through
	/// constructor (IMO the
	/// best option, because you
	/// strictly specify for
	/// user what you must have
	/// for allocating a such
	/// class, instead of
	/// deffered initialization
	/// like
	/// AddLoaderByExtension, it
	/// is just an example
	/// naming and an example
	/// approach we don't strict
	/// you there at all)
	///
	/// So how to imagine what
	/// was written about?
	///
	/// As a standard we provide
	/// one class per enum. It
	/// means if I have
	/// eResourceLoadingType::kModel,
	/// it means I can provide
	/// only one loader for this
	/// set of files (formats).
	/// But of course some users
	/// want to support many
	/// formats and these
	/// formats can have
	/// different loading
	/// algorithms. For that
	...
	/// 'general' class.
	///
	/// Let's on minimalistic
	/// example:
	/// @code
	/// class
	/// MyGeneralLoaderFor_kModel_Enum
	/// : public
	...
	/// show the idea
	/// return
	/// this->m_loaders.at(extension_of_file);
	///			}
	///
	/// private:
	...
	///
	///
	/// @param extension_of_file
	/// it is an extension of
	/// file (e.g.
	/// '.obj;.gltf;.txt;.png;.jpg;.mp4;
	/// etc'), but important,
	/// the string must be with
	/// DOT! So you can't pass
	/// just the name of
	/// extension, you need to
	/// pass the '.some_name'.
	/// Currently engine doesn't
	/// support files without
	/// formats.
	/// @return If user
	/// registered loader it
	/// would return your
	/// registered loader that
	/// mapped to specified
	/// extension. For example
	/// if you set a key to your
	/// map as '.txt' and set
	/// the value as new
	/// ktkMyLoaderForTxTExampleName
	/// so as the result of
	/// overriding of this
	/// method you must get your
	/// instance of
	/// ktkMyLoaderForTxTExampleName
	/// by passing
	/// '.txt' string to that
	...
	/// any other
	/// container/method/methodology
	/// for returning your
	/// instance that
	/// corresponds to passed
	/// argument)
	///
	/// @see
	/// Kotek::Render::ktkLoaderModel_CGLTF
	/// default implementation
	/// of cgltf loader library
	virtual ktkIResourceLoader*
	Get_Loader(const ktk_filesystem_path& extension_of_file
	) noexcept = 0;

	virtual ktk::cstring Get_AllSupportedFormats(void
	) const noexcept = 0;
};
...
public:
	...
	virtual void
	Initialize(ktkIFileSystem*, ktkMainManager*) = 0;
	virtual void Shutdown(void) = 0;

	virtual void Set_Loader(
		eResourceLoadingType resoruce_type,
		ktkIResourceLoader* p_loader
	) = 0;

	virtual ktkIResourceLoader*
	Get_Loader(eResourceLoadingType resource_type
	) const noexcept = 0;
...
public:
	...
	virtual bool Save(
		const ktk_filesystem_path& path,
		kun_core ktkResourceHandle object_for_saving
	) noexcept = 0;
	...
	virtual ktkIResourceSaver*
	Get_Saver(const ktk_filesystem_path& extension_of_file
	) noexcept = 0;
...
public:
	 ...
	 * in order to being called
	 * from child destructors
	 *
	 */
	...
	virtual void Set_Saver(
		...
		ktkIResourceSaver* p_saver
	) = 0;
	virtual ktkIResourceSaver*
	Get_Saver(eResourceLoadingType resource_type
	) const noexcept = 0;

	virtual bool Save(
		const ktk_filesystem_path& path,
		kun_core ktkResourceHandle data
	) noexcept = 0;
...
public:
    ktkResourceWritingData() :
        m_type{eResourceWritingDataType::kUnknown},
m_resource_id{}, m_size{},
m_data{}
    {
    }
    ~ktkResourceWritingData() {}

    eResourceWritingDataType
Get_Type(void) const noexcept
    ...
    void
Set_Type(eResourceWritingDataType
type) { this->m_type = type; }

    kun_core ktkResourceHandle
Get_Data(void) const noexcept
    {
        return this->m_data;
    }

    void Set_Data(kun_core
ktkResourceHandle data) {
this->m_data = data; }

    ktk::size_t
Get_ElementCount(void) const
noexcept { return this->m_size;
}

    void
Set_ElementCount(ktk::size_t
size) { this->m_size = size; }

    void
Set_ResourceID(ktk::uint32_t id)
{ this->m_resource_id = id; }

    ktk::uint32_t
Get_ResourceID(void) const
noexcept
    {
        return
this->m_resource_id;
...
private:
    /// \~english @brief for
casting eResourceWritingDataType
m_type; ktk::uint32_t
m_resource_id; ktk::size_t
m_size; kun_core
ktkResourceHandle m_data;
};
*/
...
public:
	...
	virtual kun_ktk shared_ptr<ktkResourceHandle>
	Load(const ktkLoadingRequest& request) noexcept = 0;
	...
	virtual void Close_FileStream(kun_ktk cfstream* p_fstream
	) noexcept = 0;

	virtual void
	Set_ResourceLoader(ktkIResourceLoaderManager* p_instance
	) noexcept = 0;

	virtual ktkIResourceLoaderManager* Get_ResourceLoader(void
	) const noexcept = 0;

	virtual void
	Set_ResourceSaver(ktkIResourceSaverManager* p_instance
	) noexcept = 0;

	virtual ktkIResourceSaverManager* Get_ResourceSaver(void
	) const noexcept = 0;

	virtual void Set_RenderResourceManager(
		ktkIRenderResourceManager* p_instance
	) noexcept = 0;

	virtual ktkIRenderResourceManager*
	Get_RenderResourceManager(void) const noexcept = 0;

	virtual void Set_MainManager(ktkMainManager* p_instance
	) noexcept = 0;
	virtual ktkMainManager* Get_MainManager(void
	) const noexcept = 0;
