...
namespace gl
{
	...
	void kotek_render_allocator_ring_with_tabs::OnBeginFrame(void) noexcept
	{
		this->m_p_allocated_memory_per_frame_buffer
			[this->m_frame_buffer_index] = this->m_memory_allocated_in_frame;
		this->m_memory_allocated_in_frame = 0;

		this->m_frame_buffer_index =
			(this->m_frame_buffer_index + 1) % this->m_frame_buffer_number;
	...
	void kotek_render_allocator_ring::create(ktk::uint32_t total_size) noexcept
	{
		this->m_total_size = total_size;
	}

	ktk::uint32_t kotek_render_allocator_ring::makePaddingToAvoidCrossover(
	...
	ktk::uint32_t kotek_render_allocator_ring::getSize(void) const noexcept
	{
		return this->m_allocated_size;
	}

	ktk::uint32_t kotek_render_allocator_ring::getHead(void) const noexcept
	{
		return this->m_head;
	}

	ktk::uint32_t kotek_render_allocator_ring::getTail(void) const noexcept
	{
