...
vector4f::vector4f(const vectornf_view_t& view)
{
#ifdef KOTEK_USE_MATH_LIBRARY_IMPLICIT_CASTING
	#error todo: impl
#else
	...
	KOTEK_ASSERT(view.get_row_count() == get_row_count(), "wrong dimension");
	...
	if (view.data())
	{
		if (view.get_row_count() == get_row_count())
		{
			...
			this->m_base.z = view.z();
			this->m_base.w = view.w();
		}
	}
#endif
}

vector4f::vector4f(const vectornf_const_view_t& view) 
{
#ifdef KOTEK_USE_MATH_LIBRARY_IMPLICIT_CASTING
	#error todo: impl
#else
...
math_id_t vector4f::size_of(void) noexcept
{
	static_assert(
...
math_id_t vector4f::get_column_count(void
) const noexcept
{
	return 1;
}

math_id_t vector4f::get_row_count(void) const noexcept
{
	return 4;
}
