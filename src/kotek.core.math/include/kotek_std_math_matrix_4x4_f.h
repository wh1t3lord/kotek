...
) noexcept
{
	...
	// (L*R)^T == R^T * L^T, hence the swapped operand order
...
operator*(const matrix4x4f& left, const vector4f& vec) noexcept
{
	...
	vector4f casted;
	DirectX::XMStoreFloat4(
		reinterpret_cast<DirectX::XMFLOAT4*>(&casted.Get_Base()),
		result
	);
