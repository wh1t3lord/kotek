...
using ktkUserCallbackShutdown =
	bool(kun_kotek kun_core ktkMainManager*);
using ktkUserCallbackUpdate =
	void(kun_kotek kun_core ktkMainManager*);
using ktkUserCallbackInitializeRender =
	bool(kun_kotek kun_core ktkMainManager*);
...
Kotek::ktk::function<ktkUserCallbackUpdate>
...
namespace Game
{
	bool registerCommands(Core::ktkMainManager* p_main_manager
	) noexcept
	{
		return true;
	}
	...
	bool
	InitializeModule_Game(Core::ktkMainManager* p_main_manager)
	{
...
#ifdef KOTEK_USE_DEVELOPMENT_TYPE_SHARED
		/* todo: delete since we read and init engine config
		   long before coming to this place of execution
		        ktk_filesystem_path path_to_system_json;
		        //
		   p_main_manager->GetFileSystem()->GetFolderByEnum(
		        ...
		        p_main_manager->GetFileSystem()->Make_Path(
		            path_to_system_json,
		            kun_core eFolderIndex::kFolderIndex_Root
		        );
		        ...
		        Core::ktkResourceLoaderFile_Text
		   resource_loader_text( p_main_manager
		        );

		        auto status = resource_loader_text.Load(
		            path_to_system_json,
		            kun_core ktkResourceHandle(&file, true)
		        );
		        KOTEK_ASSERT(
		            status,
		            "can't load text: {}",
		        ...
		        const auto& field_initialize_callback_name =
		            file.Get<ktk::ustring>(
		                Core::kSysInfoFieldName_InitializeCallback
		            );
		        const auto& field_shutdown_callback_name =
		            file.Get<ktk::ustring>(
		                Core::kSysInfoFieldName_ShutdownCallback
		            );
		        const auto& field_update_callback_name =
		            file.Get<ktk::ustring>(
		                Core::kSysInfoFieldName_UpdateCallback
		            );

		        const auto& field_library_name =
		   file.Get<ktk::ustring>(
		            Core::kSysInfoFieldName_UserLibraryName
		        );
		        const auto&
		...
		/*
		        KOTEK_ASSERT(
		            field_library_name.empty() == false,
		            "you can't have an invalid name of library,
		   " "because you need " "to load it!!!"
		        );*/

		kun_core ktkIFrameworkConfig* p_config =
			p_main_manager->Get_EngineConfig();

		KOTEK_ASSERT(
			p_config,
			"must be initialized ktkIFrameworkConfig instance!"
		);
		...
		auto* p_user_dll =
			static_cast<ktk::dll::shared_library*>(
				p_main_manager->Get_EngineConfig()
					->Get_UserLibrary()
			);
		...
		KOTEK_ASSERT(
			...
			p_config->Get_UserLibrary_Name(),
			p_config->Get_UserLibrary_CallbackName(
				kun_core eUserEngineLibraryCallbacks::kGame_Init
			)
		);

		KOTEK_ASSERT(
			p_user_callback_shutdown_game_library != nullptr,
			"can't obtain function from game user library {}: "
			"{}",
			p_config->Get_UserLibrary_Name(),
			p_config->Get_UserLibrary_CallbackName(
		...
		KOTEK_ASSERT(
			p_user_callback_update_game_library != nullptr,
			"can't obtain function from game user library {}: "
			"{}",
			p_config->Get_UserLibrary_Name(),
			p_config->Get_UserLibrary_CallbackName(
		...
		::KOTEK_USER_FUNCTION_INITIALIZE_MODULE_GAME(
			p_main_manager
		);
		KOTEK_ASSERT(
			p_main_manager->GetGameManager(),
	...
	bool
	ShutdownModule_Game(Core::ktkMainManager* p_main_manager)
	{
#ifdef KOTEK_USE_DEVELOPMENT_TYPE_SHARED
...
#elif defined(KOTEK_USE_DEVELOPMENT_TYPE_STATIC)
		::KOTEK_USER_FUNCTION_SHUTDOWN_MODULE_GAME(
			p_main_manager
		);
#else
	#error Unknown development type, see what you specified in your cmd/cmake GUI. The macro accepts only two variables STATIC or SHARED
...
{
	void
	ValidateMainManager(Core::ktkMainManager* p_main_manager
	) noexcept
	{
		...
		KOTEK_ASSERT(
			...
			"you didn't initialize filesystem manager field in "
			"main "
			"manager"
		);
		KOTEK_ASSERT(
			p_main_manager->Get_Input(),
		...
		if (p_main_manager->Get_EngineConfig()
		        ->IsContainsConsoleCommandLineArgument(
					kConsoleCommandArg_Editor
				) == false)
		{
			KOTEK_ASSERT(
				p_main_manager->getRenderDevice(),
				"you didn't initialize render device manager "
				"field in main "
				"manager"
			);
			KOTEK_ASSERT(
				p_main_manager->GetRenderResourceManager(),
				"you didn't initialize render resource manager "
				"in main "
				"manager"
			);
			KOTEK_ASSERT(
				p_main_manager->getRenderSwapchainManager(),
				"you didn't initialize render swapchain "
				"manager in main "
				"manager"
			);
		}
		...
		KOTEK_ASSERT(
			p_main_manager->GetGameManager(),
			"you didn't initialize game manager in your game "
			"library or "
	...
	void PrintCompiler(void) noexcept
	{
		...
		KOTEK_MESSAGE(
			"Compiled with: Clang {}", __clang_version__
		);
	...
	#elif (_MSC_VER == 1300)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio .NET 2002 (7.0)"
		);
	#elif (_MSC_VER == 1310)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio .NET 2003 (7.1)"
		);
	#elif (_MSC_VER == 1400)
	...
	#elif (_MSC_VER == 1910)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2017 RTW (15.0)"
		);
	#elif (_MSC_VER == 1911)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2017 version 15.3"
		);
	#elif (_MSC_VER == 1912)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2017 version 15.5"
		);
	#elif (_MSC_VER == 1913)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2017 version 15.6"
		);
	#elif (_MSC_VER == 1914)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2017 version 15.7"
		);
	#elif (_MSC_VER == 1915)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2017 version 15.8"
		);
	#elif (_MSC_VER == 1916)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2017 version 15.9"
		);
	#elif (_MSC_VER == 1920)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 RTW (16.0)"
		);
	#elif (_MSC_VER == 1921)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 version 16.1"
		);
	#elif (_MSC_VER == 1922)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 version 16.2"
		);
	#elif (_MSC_VER == 1923)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 version 16.3"
		);
	#elif (_MSC_VER == 1924)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 version 16.4"
		);
	#elif (_MSC_VER == 1925)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 version 16.5"
		);
	#elif (_MSC_VER == 1926)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 version 16.6"
		);
	#elif (_MSC_VER == 1927)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2019 version 16.7"
		);
	#elif (_MSC_VER == 1928)
		KOTEK_MESSAGE("Compiled with: Visual Studio 2019 "
		              "version 16.8, 16.9");
	#elif (_MSC_VER == 1929)
		KOTEK_MESSAGE("Compiled with: Visual Studio 2019 "
		              "version 16.10, 16.11");
	#elif (_MSC_VER == 1930)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 RTW (17.0)"
		);
	#elif (_MSC_VER == 1931)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.1"
		);
	#elif (_MSC_VER == 1932)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.2"
		);
	#elif (_MSC_VER == 1933)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.3"
		);
	#elif (_MSC_VER == 1934)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.4"
		);
	#elif (_MSC_VER == 1935)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.5"
		);
	#elif (_MSC_VER == 1936)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.6"
		);
	#elif (_MSC_VER == 1937)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.7"
		);
	#elif (_MSC_VER == 1938)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.8"
		);
	#elif (_MSC_VER == 1939)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.9"
		);
	#elif (_MSC_VER == 1940)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.10"
		);
	#elif (_MSC_VER == 1941)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.11"
		);
	#elif (_MSC_VER == 1942)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.12"
		);
	#elif (_MSC_VER == 1943)
		KOTEK_MESSAGE(
			"Compiled with: Visual Studio 2022 version 17.13"
		);
	#elif (_MSC_VER == 1944)
		KOTEK_MESSAGE(
		...
		if (p_manager)
		{
			if (p_manager->Get_ARGC() > 0)
			{
				if (p_manager->Get_ARGV())
				{
					...
					for (char i = 0; i < p_manager->Get_ARGC();
					     ++i)
					{
	...
	bool InitializeEngine(Core::ktkMainManager* p_main_manager)
	{
		...
		bool is_need_to_show_splash_window =
			Hide_SplashWindow(p_main_manager);
		...
		if (!is_need_to_show_splash_window)
		{
			...
			kun_kotek kun_ktk kun_mt thread thread_splash(
				[p_main_manager]()
				{
					if (p_main_manager)
					{
						kun_kotek kun_core ktkIWindowSplash*
							p_window =
		...
		// initializes and validates and does nothing with write
		// and read operations
	...
	#error Unknown development type, see what you specified in your cmd/cmake GUI. The macro accepts only two variables STATIC or SHARED
		...
		int argc =
			p_main_manager->Get_EngineConfig()->GetARGC();
