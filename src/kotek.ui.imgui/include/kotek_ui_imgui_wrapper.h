...
public:
	...
	void
	PushStyleColor(ImGuiCol idx, const ImVec4& col) override;
	void PopStyleColor(int count = 1) override;
	void PushStyleVar(ImGuiStyleVar idx, float val) override;
	void
	PushStyleVar(ImGuiStyleVar idx, const ImVec2& val) override;
	...
	void
	PushTextWrapPos(float wrap_local_pos_x = 0.0f) override;
	...
	ImU32
	GetColorU32(ImGuiCol idx, float alpha_mul = 1.0f) override;
	...
	void
	LabelText(const char* label, const char* fmt, ...) override;
	void LabelTextV(
	...
	bool
	TreeNode(const char* str_id, const char* fmt, ...) override;
	bool
	TreeNode(const void* ptr_id, const char* fmt, ...) override;
	bool TreeNodeV(
	...
	void
	SetNextItemOpen(bool is_open, ImGuiCond cond = 0) override;
	bool Selectable(
		const char* label,
		bool selected = false,
	...
	bool
	BeginMenu(const char* label, bool enabled = true) override;
	...
	TableGetColumnFlags(int column_n = -1) override;
	...
	void
	SetColumnOffset(int column_index, float offset_x) override;
	...
	bool
	BeginDragDropSource(ImGuiDragDropFlags flags = 0) override;
	...
	bool
	IsItemClicked(ImGuiMouseButton mouse_button = 0) override;
	...
	bool
	IsMousePosValid(const ImVec2* mouse_pos = NULL) override;
	...
	void
	ResetMouseDragDelta(ImGuiMouseButton button = 0) override;
