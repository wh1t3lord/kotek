...
enum class eFolderIndex : kun_ktk enum_base_t{
	kFolderIndex_Root,
	kFolderIndex_DataGame,
	kFolderIndex_DataGame_Configs,
	kFolderIndex_DataGame_Scripts,
	kFolderIndex_DataGame_Textures,
	kFolderIndex_DataGame_Shaders,
	kFolderIndex_DataGame_Shaders_GLSL,
	kFolderIndex_DataGame_Shaders_HLSL,
	kFolderIndex_DataGame_Shaders_WEBGPU,
	kFolderIndex_DataGame_Shaders_SPV,
	kFolderIndex_DataGame_Models,
	kFolderIndex_DataGame_Sounds,
	kFolderIndex_DataGame_Levels,
	kFolderIndex_DataGame_AI,
	kFolderIndex_DataUser_Tests,
	kFolderIndex_DataUser_ShaderCache,
	kFolderIndex_DataUser_SDK,
	kFolderIndex_DataUser_SDK_Settings,
	kFolderIndex_DataUser_SDK_Scenes,
	kFolderIndex_DataUser,
	kEndOfEnum
};
...
KOTEK_IMPLEMENTATION_ENUM_FLAG_OPERATORS(
	eFileSystemType, kun_ktk uint16_t
);

enum class eFileSystemPriorityType : kun_ktk uint8_t{
	/// @brief will use what user passed to
	/// ktkFileSystem::Initialize
	kAuto,
	kNative,
	kZlib,
	kEndOfEnum = kZlib
...
enum class eFileSystemFeatureType : kun_ktk uint16_t{
	kNone = 0,
	kVFMRead = 1 << 1,
	// todo: think about it and implement if needed but it is
    // hard tbh :c
	kVFMWrite = 1 << 2,
	/// @brief \~english if this flag was specified as features
    /// then priority (eFileSystemPriorityType) will take action
...
enum class eResourceRequestResourceType : kun_ktk enum_base_t{
	kText,
	kTexture,
	kModel,
...
enum class eResourceRequestThreadingPolicy : kun_ktk
	enum_base_t{kAsync, kSync, kEndOfEnum};
...
/// \~russian @brief Данное перечисление определяет какие
/// возможности имеет движок.
///
/// @warning Важно понимать, что данное перечисление было
/// создано только, чтобы отмечать какие функции сейчас работают
/// в движке, но это не значит что включая ту или иную
/// возможность она реально включается. Как это понимать? Ещё
/// стоит отметить, что определять когда мы реально должны
/// указывать то или иное поле из перечисления зависит от
/// аргументов переданных в само приложение. К примеру есть флаг
/// '--sdk', и пользователь его передал приложению, в движке мы
/// можем считать а передал такой флаг пользователь нашему
/// приложению? Если да, то мы должны поставить true
/// соответствующему полю (в данном случае kEngine_Feature_SDK),
/// однако это надо делать только в том случае, когда мы вызвали
/// и реализовали все функции отвечающие за инициализацию СДК,
/// иначе это не имеет никакого смысла, мы говорим пользователю
...
/// - Когда конечный пользователь определяет статус включено ли
/// поле, если включено, то Пользователь гарантирует, что все
/// функции были вызваны в движке и они работают.
///
/// Рассмотрим пример:
///
/// @code
...
/// пользователю даем понять какая функция в движке работает.
/// Побольшей части это и есть какое-то подобие конфига, в том
/// смысле, что мы определяем на логическом уровне, что у нас
/// есть, а что у нас нет. Чтобы уже на уровне выполнения кода
/// мы могли обрабатывать условия в динамике, к примеру сейчас
/// включена такая "возможность"/функция, а потом вдруг мы
/// захотели ее отключить и соответственно все места должны
/// иметь покрытие на уровне p_config->IsFeatureEnabled и если
/// true то исполняем какую-то логику. То есть это формальное
/// обозначение того факта, что та или иная реализация
/// отработала и как бы она работает, то есть включена.
...
///			// Если такая функция была включена, то это значит
/// что
/// пользовать реализовал инициализацию и вызвал все необходимые
/// для этого функции, то есть включена возможность = работает в
/// движке прямо сейчас.
///
///		}
///
...
/// features the engine supports for runtime usage.
///
/// @warning You need to understand the purpose of this
...
/// }
///
/// // somewhere in SDK handlers instances like SDK resource
/// manager or some window
///
/// bool ExecuteSomeCode_ButCheckIfSDKFeatureIsEnabled() {
///		Kotek::Core::ktkIFrameworkConfig* p_config =
/// p_main_manager->Get_EngineConfig();
///
///		if (p_config->IsFeatureEnabled(kEngine_Feature_SDK)) {
///			// do some real action here, because SDK
/// initialization was
/// called somewhere
///
///		}
...
enum class eEngineFeature : kun_ktk uint32_t{
	/// \~english @brief This field is system and it means that
	/// if the
	/// required feature is not implemenented or exist you need
	/// to use
	/// this field. For example, the user asks for determining
	/// if
	/// kEngine_Feature_SomethingThatDoesntExistInEnum exists in
	/// engine.
	/// But we can't return anything else as false state, and
	/// for
	/// obtaining the field if the user asks for the field we
	/// will
	...
	/// для того,
	/// чтобы сказать что запрашиваемое поле не существует
	/// вообще, то
	/// есть в самом перечислении. В таком случае разработчик
	/// должен
	/// воспользоваться данным полем, чтобы сказать пользователю
	/// что его
	/// запрашиваемое поле не существует и неизвестно для
	/// системы.
	///
...
KOTEK_IMPLEMENTATION_ENUM_FLAG_OPERATORS(
	eEngineFeature, kun_ktk uint32_t
);

enum class eEngineFeatureWindow : kun_ktk uint32_t{
	/// \~english @brief This field specifies that (main) window
	/// is in
	/// windowed mode. So if user checks if this feature is in
	/// true
	/// state, so it means the window is in windowed mode. The
	/// developer
	/// must check that the field sets to true only if the
	/// window is in
	/// real windowed mode.
	/// \~russian @brief Данное поле означает, что (главное)
	/// окно
	/// приложения находится в оконном режиме. Для конечного
	/// пользователя это означает, что если он хочет проверить
	/// данное
	/// поле, то если оно включено, значит окно реально
	/// находится в
	/// оконном режиме, разработчик должен убедиться и
	/// гарантировать,
	/// что если поле включено, то это соответствует
	/// действительности.
	/// \~german @brief
	/// \~french @brief
	...
	/// borderless mode. So if the user checks if this field is
	/// set to
	/// true state, it means that window is in borderless mode.
	/// The
	/// developer must check that window is indeed in borderless
	/// mode
	/// and set this field in true state for notifying user.
	/// \~russian @brief Данное поле означает, что окно
	/// находится в
	/// оконном режиме, но без оконных рамок. Конечный
	/// пользователь при
	/// проверке должен получать состояние, что окно реально
	/// находится в
	/// соответствующем режиме. За этим должен следить
	/// разработчик и
	...
	/// \~english @brief This field specifies that window is in
	/// fullscreen mode. If user checks that field is in true
	/// state it
	/// means that window is in real fullscreen mode. The
	/// developer must
	/// provide to user that window is in fullscreen and the
	/// appropriate
	/// is in true state.
	/// \~russian @brief Данное поле показывает что окно
	/// находится в
	/// полноэкранном режиме. Это означает, что если
	/// пользователь
	/// проверяет данное поле что оно включено, то это значит
	/// что окно
...
enum class eEngineFeatureSDK : kun_ktk uint32_t{
	/// \~english @brief This field means that your engine is
	/// called SDK
	/// initialization or it means that your SDK implementation
	/// is
	/// working in Engine. So for user it means that if your
	/// status of
	/// this field equals true that means that your SDK is
	/// enabled and
	/// working in Engine.
	///
	/// \~russian @brief Данное поле обозначает что ваш СДК
	/// работает и
	/// все необходимые процедуры для создания были вызваны в
	/// движке.
	/// Для конечного пользователя будет означать следующее, что
	/// если
	/// это поле равно true, то пользователь может обрабатывать
	/// ситуации
	...
	/// \~english @brief This field means that your engine
	/// implemented
	/// SDK based on ImGui. So for end user the checking status
	/// of this
	/// feature will be meant that you implemented and called
	/// all things
	/// that related for Initialization/Creation SDK which is
	/// based on
	/// ImGui and it means that it works in Engine right now.
	/// \~russian @brief Данное поле означает, что вы имеете
	/// реализацию
	/// SDK основанного на парадигме ImGui. Для конечного
	/// пользователя
...
KOTEK_IMPLEMENTATION_ENUM_FLAG_OPERATORS(
	eEngineFeatureSDK, kun_ktk uint32_t
);
...
enum class eEngineFeatureRenderer : kun_ktk uint32_t{
	/// \~english @brief This field means that engine uses
	/// OpenGL
	/// version 4.6. If this field is true it means that engine
	/// uses
	/// OpenGL 4.6 currently. The developer must check the valid
	/// state
	/// of this field in order to provide it to end user.
	/// \~russian @brief Данное поле обозначает, что движок
	/// использует
	/// рендерер OpenGL версии 4.6 (то есть текущую самую
	/// последнюю). Если поле
	/// имеет true состояние, то это значит что движок
	/// использует данный
	/// рендерер OpenGL 4.6. Разработчик должен гарантировать
	/// что состояние поля
	...
	/// \~english @brief This field means that renderer is
	/// Vulkan. In
	/// Engine it supposed that only one renderer can exist,
	/// thus if
	/// have a true state for this field it means that engine
	/// currently
	/// uses Vulkan renderer. The developer must provide a valid
	/// state
	/// when the Vulkan renderer is initialized and set state
	/// for this
	/// field as true.
	/// \~russian @brief Данное поле обозначает, что движок
	/// работает с
	/// рендерером Vulkan, при условии что само значение поле
	/// true.
	/// Разработчик должен следить за тем, что состояние этого
	/// поля
	...
	/// \~english @brief This field means that renderer is
	/// software. In
	/// Engine it supposed that only one renderer can exist, so
	/// if you
	/// have this field as state true that means that engine is
	/// currently working with Software Renderer. The developer
	/// must
	/// provide a valid state when this field is true.
	/// \~russian @brief Данное поле означает, что текущий
	/// рендерер
	/// работает только на ЦПУ без поддержки ГПУ.
	/// Предполагается, что
	/// движок может работать только с одним рендерером, а это
	/// значит,
	/// что если данное поле включено, то это значит движок
	/// работает с
	/// ЦПУ рендерером. Разработчик должен гарантировать, что
	/// если
	/// пользователь получил состаяние true, то и движок
	/// работает с
	/// данным рендерером.
	/// \~german @brief
	/// \~french @brief
...
enum class eEngineFeatureRender : kun_ktk uint32_t{
	/// \~english @brief This field means that renderer enabled
	/// MSAA
	/// feature. If the field's state is true it means that
	/// renderer
	/// implemented and enabled MSAA feature. The developer must
	/// set the
	/// valid state of field.
	/// \~russian @brief Данное поле означает что рендерер
	/// включил
	/// возможность MSAA. Если состояние поля true означает что
	/// разработчик реализовал и включил MSAA. Разработчик
	/// должен
	/// гарантировать что состояние поля соответствует работе
	/// рендерера.
	/// \~german @brief
	/// \~french @brief
	...
	/// feature. If the field's state is true it means that
	/// renderer
	/// implemented and enabled VSYNC feature. The developer
	/// must set
	/// the valid state of field.
	/// \~russian @brief Данное поле означает что рендерер имеет
	/// VSYNC
	/// функцию. Если поле имеет состояние true это означает,
	/// что
	/// реднерер реализовал и включил VSYNC функцию. Разработчик
	/// должен
	/// гарантировать что состояние поля соответствует работе
	/// рендерера.
	/// \~german @brief
...
/**
 * \~english @brief This is used for console manager class
 * ktkConsole. And it uses for define callback functions when we
 * want to issue some command by enum. For that case you use
 * ktkConsole::Execute or ktkConsole::PushCommand. The
 * difference between two that one executes right now the second
 * just pushes to queue and executes on next tick.
 */
enum class eConsoleCommandIndex : kun_ktk enum_base_t{
	...
	kConsoleCommand_App_Resize, // todo: implement this, note
								// that render_resize
	...
	/**
     * \~english @brief Defines functionality to your log window
     * when issued it must send message to log
     */
	kConsoleCommand_SDK_SendMessageToLogWindow,

	/**
     * \~english @brief Sends warning message (in my opinion it
     * could be yellow color) to log window so it depends on
     * your implementation how you will do that in your
     * 'business logic'
     */
	kConsoleCommand_SDK_SendMessageWarningToLogWindow,

	/**
     * \~english @brief Sends error message (in my opinion it
     * could be red color) to log window so it depends on your
     * implementation how you will do that in your 'business
     * logic'
     */
	kConsoleCommand_SDK_SendMessageErrorToLogWindow,

	/**
     * \~english @brief Sends info message (in my opinion it
     * could be blue color) to log window so it depends on your
     * implementation how you will do that in your 'business
     * logic'
     */
	...
	/**
     * \~english @brief Sends grey message to log window so it
     * depends on your implementation how you will do that in
     * your 'business logic'. Of course we could define a such
     * enum like SendNotImportantMessageToLogWindow but you know
     * it is really hard to define from that perspective it is
     * just grey and user can define in his engine what he wants
     * to see it could be not important or message from physics
     * or anything else
     */
	kConsoleCommand_SDK_SendMessageGreyToLogWindow,

	/**
     * \~english @brief As you see it means we want to show
     * modal window that define that window that asks us about
     * what we want to do with current scene save and close or
     * just close scene without saving what we have in all
     * editors about editing something we can save and close our
     * application or we can close our application without
     * saving our editing instance
     */
	kConsoleCommand_SDK_ShowModalWindow_SaveAndCloseOrCloseScene,

	/**
     * \~english @brief it means that we have some component
     * like component_camera, but we say that component_camera
     * can't exist without component_input because it is
     * pointless to have camera input especially in your editor,
     * so for that case define components that are required for
     * creation of component_camera in our case it is
     * component_input. So when we want to add component_camera
     * have two ways like we just print warning message that we
     * can't create that component because you don't have the
     * required one (e.g. component_input) or we can
     * automatically can components that are required for
     * creation so it means when we add component_camera we
     * automatically create component_input without any
     * notifications so sometimes it could be useful, but user
     * doesn't to enable a such feature like we provide a such
     * thing that he always must see what components are
     * required. But I guess it is pointless to have a such
     * thing like you need always manually add required
     * components instead of 'automatization', but who knows who
     * wants to add a such things through this way...
     *
     * @warning You need to interpret that like you enable or
     * disable feature so it doesn't mean that it issues some
     * code that adds components to entity, no it is about some
     * flag enable/disable state.
     */
...
// TODO: not obvious and may conflict with input system better
// to rename to something like eInputCursorType otherwise
// specify working field where that enum relates to
...
enum class eFileWritingControlCharacterType : kun_ktk
	enum_base_t{/// @brief just a white space aka " "
                kSpace,
                /// @brief aka std::endl
                kNewLine,
                /// @brief aka std::flush
...
enum eInputControllerType
{
	...
	kControllerUnknown =
		kControllerTotalAmountOfEnum // reserved for unspecified
	                                 // devices
};
...
KOTEK_IMPLEMENTATION_ENUM_FLAG_OPERATORS(
	eInputMouseKeys, kun_ktk uint16_t
);

/// @brief CK = ControllerKeyboard; CM = ControllerMouse; CG =
/// ControllerGamepad; CJ = ControllerJoystick; CA =
/// ControllerAny;
enum eInputAllKeys
{
	kCK_KEY_A,
...
enum class eInputKeyboardKeys : kun_ktk uint32_t{
	kKEY_A = 1 << 1,          kKEY_B = 1 << 2,
	kKEY_C = 1 << 3,          kKEY_D = 1 << 4,
	kKEY_E = 1 << 5,          kKEY_F = 1 << 6,
	kKEY_G = 1 << 7,          kKEY_H = 1 << 8,
	kKEY_I = 1 << 9,          kKEY_J = 1 << 10,
	kKEY_K = 1 << 11,         kKEY_L = 1 << 12,
	kKEY_M = 1 << 13,         kKEY_N = 1 << 14,
	kKEY_O = 1 << 15,         kKEY_P = 1 << 16,
	kKEY_Q = 1 << 17,         kKEY_R = 1 << 18,
	kKEY_S = 1 << 19,         kKEY_T = 1 << 20,
	kKEY_U = 1 << 21,         kKEY_V = 1 << 22,
	kKEY_W = 1 << 23,         kKEY_X = 1 << 24,
	kKEY_Y = 1 << 25,         kKEY_Z = 1 << 26,
	kKEY_CAPS_LOCK = 1 << 27, kKEY_SCROLL_LOCK = 1 << 28,
	kKEY_UNKNOWN = 0
};

KOTEK_IMPLEMENTATION_ENUM_FLAG_OPERATORS(
	eInputKeyboardKeys, kun_ktk uint32_t
);
...
static_assert(
	static_cast<kun_ktk uint32_t>(
		eInputKeyboardKeysEnter::kKEY_ENTER_NUMPAD
	) ==
		static_cast<kun_ktk uint32_t>(
