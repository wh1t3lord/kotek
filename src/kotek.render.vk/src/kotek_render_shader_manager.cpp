...
namespace Kotek
{
	namespace Render
	{
		namespace vk
		{
			...
			void
			kotek_render_shader_manager::initialize() noexcept
			{
			}

			void kotek_render_shader_manager::shutdown(void
			) noexcept
			{
			}
			...
			kotek_render_shader_manager::loadShader(
				const ktk::ustring& path, shader_type_t type
			) noexcept
			{
				KOTEK_CPU_PROFILE();
				...
				if (this->m_p_filesystem->Is_Exists(path.c_str()
				    ) == false)
				{
					KOTEK_ASSERT(
						false,
				...
				{
#ifdef KOTEK_USE_VULKAN_SHADERC
				...
				{
					KOTEK_ASSERT(
						false, "can't detect type of shader"
					);
					break;
				}
				}
				...
				if (shader_code.empty())
				{
					KOTEK_ASSERT(
						false,
						"shader code is not valid. Can't "
						"compile and "
						"load shader by path: [{}]",
						reinterpret_cast<const char*>(
							path.c_str()
			...
			) noexcept
			{
				...
				if (path.empty())
				{
					KOTEK_MESSAGE_WARNING(
						"you passed an empty path"
					);
					return result;
				}

				if (this->m_p_filesystem->Is_Exists(path) ==
				    false)
				{
					KOTEK_MESSAGE_WARNING(
						"you passed an invalid path: [{}]",
				...
				{
					KOTEK_MESSAGE_WARNING(
						"can't load unknown shader by type"
					);
					return result;
				}
			...
			) noexcept
			{
				...
				{
					KOTEK_MESSAGE_WARNING(
						"you can't have an empty code string "
...
#ifdef KOTEK_USE_VULKAN_SHADERC
					shader_code =
						this->compileShaderFromStringToSPIRV(
							code_as_string,
							shaderc_shader_kind::
								shaderc_fragment_shader
						);
				...
				{
					KOTEK_MESSAGE_WARNING(
						"can't build shader from Vulkan API, "
						"because your "
				...
				KOTEK_ASSERT(
					p_device,
					"your device is invalid (VkDevice)"
				);

				VkShaderModule p_module = nullptr;

				VkResult status = vkCreateShaderModule(
					p_device, &info, nullptr, &p_module
				);
			...
			void kotek_render_shader_manager::destroyShader(
				const shader_module_t& instance
			) noexcept
			{
			...
			kotek_render_shader_manager::compileShaderToSPIRV(
				const ktk::cstring& path_to_file,
				shaderc_shader_kind type,
				const ktk::map<ktk::cstring, ktk::cstring>&
					macros
			) noexcept
			{
				KOTEK_CPU_PROFILE();
				...
				if (this->m_p_filesystem->Is_Exists(path_to_file
				    ) == false)
				{
					KOTEK_MESSAGE_WARNING(
						"invalid path for compiling shader to "
						"SPIRV: [{}]",
						reinterpret_cast<const char*>(
							path_to_file.c_str()
						)
				...
				for (const auto& pair_macro_name_and_value :
				     macros)
				{
			...
			ktk::vector<ktk::uint32_t>
			kotek_render_shader_manager::
				compileShaderFromStringToSPIRV(
					const ktk::cstring& code_as_string,
					shaderc_shader_kind type,
					const ktk::map<ktk::cstring, ktk::cstring>&
						macros
			...
			shader_type_t
			kotek_render_shader_manager::detectType(

				const ktk::ustring& path_to_file
			) noexcept
			{
				KOTEK_CPU_PROFILE();
				...
				{
					KOTEK_MESSAGE_WARNING(
						"invalid path to file, can't analyze "
						"this path: [{}]",
						reinterpret_cast<const char*>(
							path_to_file.c_str()
						)
				...
				if (temp.has_filename())
				{
					    ...
					    ) != ktk::ustring::npos)
					{
						...
						return shader_type_t::
							kShaderType_Vertex;
					}
					else if (temp_filename.find(
								 _kShaderPrefix_Fragment
							 ) != ktk::ustring::npos)
					{
#ifdef KOTEK_DEBUG
						KOTEK_MESSAGE(
					...
					{
						KOTEK_MESSAGE_WARNING(
							"can't detect shader type because "
							"we can't handle "
							"with this prefix or non prefixed "
							"file: [{}]",
							reinterpret_cast<const char*>(
								path_to_file.c_str()
							)
				...
				{
					KOTEK_MESSAGE_WARNING(
						"you passed an invalid path without "
						"destination file!!! [{}]",
						reinterpret_cast<const char*>(
							path_to_file.c_str()
						)
			...
			namespace helper
			{
				bool isRTXShaderByType(shader_type_t type
				) noexcept
				{
					bool is_rtx = true;
				...
				bool
				isBufferByDescriptorType(VkDescriptorType type
				) noexcept
				{
					switch (type)
					{
				...
				ktk::cstring
				translateShaderTypeToString(shader_type_t type
				) noexcept
				{
					switch (type)
					{
					...
					case shader_type_t::
						kShaderType_TessellationControl:
					{
						return "shader_type_tessellation_"
							   "control";
					}
					case shader_type_t::
						kShaderType_TessellationEvaluation:
					...
					default:
					{
						KOTEK_ASSERT(
							false,
							"can't be you passed invalid "
							"shader_type_t {} "
							"(can't parse)",
							static_cast<ktk::enum_base_t>(type)
						);

						return "shader_type_INVALID_ENUM_CANT_"
							   "PARSE";
					}
					}
				}
				...
				void printBindingInfo(
					const descriptor_set_info_t& info
				) noexcept
				{
					KOTEK_MESSAGE("\n\n/// Binding ///\n\n");

					KOTEK_MESSAGE(
						"Attached to variable (in shader): {}",
						reinterpret_cast<const char*>(
				...
				shader_type_t
				GetShaderTypeByShaderStageFlagBits(
					VkShaderStageFlagBits bits
				) noexcept
				{
					shader_type_t result =
						shader_type_t::kShaderType_Unknown;
					...
					{
						result =
							shader_type_t::kShaderType_Geometry;
					}
					else if (bits ==
					         VkShaderStageFlagBits::
					             VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT)
					{
						result = shader_type_t::
							kShaderType_TessellationControl;
					}
					else if (bits ==
					         VkShaderStageFlagBits::
					...
					{
						result =
							shader_type_t::kShaderType_Task;
					}
					else if (bits ==
					         VkShaderStageFlagBits::
				...
				VkShaderStageFlagBits
				GetShaderStageFlagBitsByShaderType(
					shader_type_t type
				) noexcept
				{
				...
				VkImageCreateInfo InitializeImageForShaders(
					int width,
					int height,
					VkImageLayout image_layout_initial,
					VkImageType image_type,
					ktk::uint32_t depth_value,
					ktk::uint32_t mip_levels,
				...
				VkImageCreateInfo InitializeImageForFramebuffer(
					int width,
					int height,
					VkImageLayout image_layout_initial,
					VkImageType image_type,
				...
				VkImageCreateInfo InitializeImage(
					int width,
					int height,
					VkImageLayout image_layout_initial,
					VkImageType image_type,
				...
				InitializePipelineRasterizationState(
					VkPolygonMode mode_polygon,
					VkCullModeFlags mode_cull,
				...
				InitializePipelineViewportState(
					ktk::uint32_t viewport_count,
				...
				InitializePipelineMultisampleState(
					VkBool32 sample_enable,
					VkSampleCountFlagBits rasterization_samples,
					float min_sample_shading,
					const VkSampleMask* p_mask,
				...
				InitializePipelineDynamicState(
			...
			kotek_render_shader_manager::shader_module_t::
				shader_module_t(void) : m_p_module(nullptr)
			{
			}
			...
			void kotek_render_shader_manager::shader_module_t::
				setSPIRVByteCode(
					const ktk::vector<ktk::uint32_t>& info
				) noexcept
			{
				KOTEK_ASSERT(
					info.empty() == false,
					"you can't set SPIRV module data as empty "
					"vector"
				);
			...
			kotek_render_shader_manager::shader_module_t::
				getSPIRVByteCode(void) const noexcept
			{
				return this->m_spv_info;
			}

			void kotek_render_shader_manager::shader_module_t::
				setModule(VkShaderModule p_module) noexcept
			{
				KOTEK_ASSERT(
					p_module,
					"you can't pass invalid pointer of "
					"VkShaderModule"
				);
				this->m_p_module = p_module;
			}

			VkShaderModule kotek_render_shader_manager::
				shader_module_t::getModule(void) const noexcept
			{
			...
			shader_loading_data_t::shader_loading_data_t(
				...
				const ktk::ustring&
					path_to_file_or_source_code_string
			) :
				m_type(type),
			...
			void shader_loading_data_t::setData(
				const ktk::variant<void*, ktk::ustring>& data
			) noexcept
			{
				this->m_data = data;
			}
			...
			shader_loading_data_t::getType(void) const noexcept
			{
				return this->m_type;
			}
			...
			{
				this->m_descriptor_set_index =
					descriptor_set_index;
			}

			ktk::uint32_t
			shader_input_data_t::getDescriptorSetIndex(void
			) const noexcept
			{
				return ktk::uint32_t();
			}
			...
			ktk::uint32_t
			shader_input_data_t::getBindingIndex(void
			) const noexcept
			{
			...
			VkDescriptorType
			shader_input_data_t::getDescriptorType(void
			) const noexcept
			{
			...
			ktk::size_t
			shader_input_data_t::GetData_BufferSize(void
			) const noexcept
			{
				KOTEK_ASSERT(
					helper::isBufferByDescriptorType(
			...
			ktk::ustring
			shader_input_data_t::GetData_RenderGraphTextureName(
				void
			) const noexcept
			{
				KOTEK_ASSERT(
			...
			const ktk::ustring&
			shader_input_data_t::GetRenderPassName(void
			) const noexcept
			{
				return this->m_render_pass_name;
			}

			bool shader_input_data_t::IsDataFor(void
			) const noexcept
			{
			...
			void
			shader_input_data_t::SetData(ktk::size_t buffer_size
			) noexcept
			{
				KOTEK_ASSERT(
					helper::isBufferByDescriptorType(
			...
			ktk::uint32_t
			descriptor_set_info_t::getDescriptorSetIndex(void
			) const noexcept
			{
			...
			ktk::uint32_t
			descriptor_set_info_t::getBindingIndex(void
			) const noexcept
			{
				return this->m_binding_index;
			}
			...
			VkDescriptorType
			descriptor_set_info_t::getDescriptorType(void
			) const noexcept
			{
				return this->m_type;
			}
			...
			const ktk::cstring&
			descriptor_set_info_t::getVariableName(void
			) const noexcept
			{
			...
			shader_type_t
			descriptor_set_info_t::GetShaderType(void
			) const noexcept
			{
