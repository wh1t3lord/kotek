...
'file' can end in .<stage> for auto-stage classification, where
<stage> is: .conf   to provide a config file that replaces the
default configuration (see -c option below for generating a
template) .vert   for a vertex shader .tesc   for a tessellation
control shader .tese   for a tessellation evaluation shader
        ...
        // NOT USED
        .glsl   for .vert.glsl, .tesc.glsl, ..., .comp.glsl
compound suffixes .hlsl   for .vert.hlsl, .tesc.hlsl, ...,
.comp.hlsl compound suffixes

        Example: You need to write the name of shader as is, but
you must specificate his content to format file

        Like if you write vertex shader you need to write output
file like this

        MyShader.vert <=== we know that is vertex shader

        But you must not write .glsl, .hlsl, .gc or other
formats of naming shader languages. Because it doesnt mean
anything and can't handle fast with analyzing them
...
namespace Kotek
{
	namespace Render
	{
		namespace vk
		{
			...
			enum class shader_loading_data_type_t : ktk::
				enum_base_t
			{
			...
			constexpr const char* _kShaderPrefix_Vertex =
				"vert";
			constexpr const char* _kShaderPrefix_Fragment =
				"frag";
			constexpr const char* _kShaderPrefix_Compute =
				"comp";
			constexpr const char* _kShaderPrefix_Task = "task";
			constexpr const char* _kShaderPrefix_Mesh = "mesh";
			constexpr const char* _kShaderPrefix_Geometry =
				"geom";
			constexpr const char*
				_kShaderPrefix_TessellationEvaluation = "tese";
			constexpr const char*
				_kShaderPrefix_TessellationControl = "tesc";
			...
			namespace helper
			{
				...
				VkImageCreateInfo InitializeImageForShaders(
					...
					ktk::uint32_t depth_value = 1,
					ktk::uint32_t mip_levels = 1,
				...
				// TODO: create support for anysotropy
				// filtering, specify it in creation when we got
				// an information about our image, we need to
				// read from config or what user specifies ???
				...
				InitializePipelineColorBlendState(
					...
					VkLogicOp logic_op = VK_LOGIC_OP_AND,
					float blend_R = 1.0f,
					float blend_G = 1.0f,
					float blend_B = 1.0f,
					float blend_A = 1.0f,
				...
				// TODO: specify all arguemnts to fill the
				// structure
			...
			public:
				...
				getData(void) const noexcept;
				void
				setData(const ktk::variant<void*, ktk::ustring>&
				            data) noexcept;

				shader_loading_data_type_t getType(void
				) const noexcept;
				void setType(shader_loading_data_type_t type
				) noexcept;
			...
			// TODO: add support for arrays data like array of
			// textures in shader
			...
			public:
				...
				ktk::uint32_t getDescriptorSetIndex(void
				) const noexcept;
				ktk::uint32_t getBindingIndex(void
				) const noexcept;

				VkDescriptorType getDescriptorType(void
				) const noexcept;

				ktk::size_t GetData_BufferSize(void
				) const noexcept;
				ktk::ustring GetData_RenderGraphTextureName(void
				) const noexcept;

				const ktk::ustring& GetRenderPassName(void
				) const noexcept;

				/// <summary>
				/// If true means for buffer otherwise it's
				/// image
				/// TODO: think about growing variant structure
				/// here, so we need enum and result status
				/// which is correctly specifies what kind of
				/// data we have here
				/// </summary>
				/// <param name=""></param>
				/// <returns></returns>
			...
			private:
				...
				void SetData(const ktk::ustring& texture_name
				) noexcept;
			...
			public:
				...
				const ktk::cstring& getVariableName(void
				) const noexcept;

				shader_type_t GetShaderType(void
				) const noexcept;
			...
			public:
				...
				public:
					...
					void setModule(VkShaderModule p_module
					) noexcept;
			...
			public:
				...
				kotek_render_shader_manager(const kotek_render_shader_manager&) =
					delete;
				kotek_render_shader_manager&
				operator=(const kotek_render_shader_manager&) =
					delete;
				kotek_render_shader_manager(kotek_render_shader_manager&&) =
					delete;
				kotek_render_shader_manager&
				operator=(kotek_render_shader_manager&&) =
					delete;
				...
				shader_module_t
				loadShader(const ktk::ustring& path) noexcept;
				...
				void
				destroyShader(const shader_module_t& instance
				) noexcept;

			private:
				/*!
				 * \brief compileShaderToSPIRV compile shader
				 * into SPIRV independent format \param
				 * path_to_file is path to file in OS
				 * \param type low level enum from shaderc
				 * library
				 * \return compiled code if it's empty vector it
				 * means we have error
				 */
				...
				shader_type_t
				detectType(const ktk::ustring& path_to_file
				) noexcept;
