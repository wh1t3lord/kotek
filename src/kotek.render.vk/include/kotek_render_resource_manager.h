...
namespace Kotek
{
	namespace Render
	{
		namespace vk
		{
			// We are queuing (backBufferCount + 0.5) frames, so
			// we need to triple buffer the resources that get
			// modified each frame
			...
			public:
				...
				ktkRenderResourceManager(const ktkRenderResourceManager&) =
					delete;
				ktkRenderResourceManager&
				operator=(const ktkRenderResourceManager&) =
					delete;
				...
				void Initialize(
					...
					ktk::size_t memory_size =
						ktk::
							kMemoryConvertValueDenominator_Megabytes *
						32
				) override;

				void
				Shutdown(Core::ktkIRenderDevice* p_raw_device
				) override;
				...
				GetDynamicBufferRing(void) noexcept;

				kotek_render_upload_heap* getUploadHeap(void
				) noexcept;

				ktkRenderTextureManager* GetTextureManager(void
				) noexcept;

				kotek_render_resource_view_heap*
				GetCreatorHeap(void) noexcept;

				kotek_render_shader_manager*
				getShaderManager(void) noexcept;

				VkRenderPass getSwapchainRenderPass(void
				) const noexcept;
				...
				const VkDescriptorPool getDescriptorPool(void
				) const noexcept;

				/*
				  VkShaderModule
				  loadShader(Core::ktkMainManager& main_manager,
				                                   const
				  ktk::ustring& path, shader_type_t type)
				  noexcept
				  {
				    return
				  this->m_manager_shader.loadShader(main_manager,
				  path, type);
				  }
				  ...
				  VkShaderModule
				  loadShaderAsString(ktkRenderDevice*
				  p_render_device, const ktk::ustring&
				  code_as_string, shader_type_t type) noexcept
				  {
				    return
				...
				VkImage
				GetSwapchainImage(ktk::uint32_t acquired_index
				) const noexcept;

				ktkRenderStats*
				Get_Statistic(Core::eRenderStatistics type
				) noexcept override;
			...
			private:
				...
				ktk::vector<VkImageView>
					m_swapchain_images_view;
				ktk::vector<VkFramebuffer>
					m_swapchain_framebuffers;

				kotek_render_static_buffer_pool
					m_static_buffer_cpu_access;
				kotek_render_static_buffer_pool
					m_static_buffer_gpu_access_only;
				kotek_render_resource_view_heap
					m_resource_view_heap;
				kotek_render_dynamic_buffer_ring
					m_dynamic_buffer_ring;
