...
void ktkRenderResourceManager::initialize(
	...
	kun_core ktkIRenderSwapchain* p_raw_swapchain, kun_ktk size_t memory_size)
{
	...
	auto tex_mem =
		kun_ktk align_down<kun_ktk size_t>((((memory_size / 100) * 70)), 2);
	this->m_p_render_manager_texture->Initialize(tex_mem);

	total_memory -= tex_mem;

	auto shader_manager_memory =
		kun_ktk align_down<kun_ktk size_t>((((memory_size / 100) * 10)), 2);
...
void ktkRenderResourceManager::shutdown(kun_core ktkIRenderDevice* p_raw_device)
{
	KOTEK_ASSERT(p_raw_device, "you can't pass an invalid interface");
...
void ktkRenderResourceManager::Resize(kun_core ktkIRenderDevice* p_raw_device,
...
ktkRenderResourceManager::LoadGeometry(
	kun_ktk enum_base_t resource_loading_type, kun_ktk uint32_t id)
{
	kun_ktk shared_ptr<kun_core ktkResourceHandle> p_result;
	...
	switch (type)
	{
	case kun_core eResourceLoadingType::kModelStatic_Triangle:
	{
		if (this->m_p_render_manager_geometry)
		{
			...
			this->m_p_render_manager_geometry->AddForUpload(
				p_geometry->Get_VertexData(), p_geometry->Get_IndexData(), id);

			p_result = std::make_shared<kun_core ktkResourceHandle>(p_geometry
#ifdef KOTEK_DEBUG
				,
	...
	case Kotek::kun_core eResourceLoadingType::kModelStatic_Box:
	{
		if (this->m_p_render_manager_geometry)
		{
		}
...
kun_ktk shared_ptr<kun_core ktkResourceHandle>
ktkRenderResourceManager::LoadGeometry(kun_ktk enum_base_t resource_loading_type,
	const ktk_filesystem_path& path_to_file, kun_ktk uint32_t id)
{
	return nullptr;
}
