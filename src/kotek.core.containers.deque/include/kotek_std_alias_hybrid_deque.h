...
namespace _kotek_hd_container_namespace = ::boost;

namespace _kotek_hd_container_namespace_pmr = ::std::pmr;

namespace _kotek_hd_container_namespace_swap = ::std;

namespace _kotek_hd_container_namespace_conditional = ::std;

namespace _kotek_hd_container_namespace_enable_if = ::std;

namespace _kotek_hd_container_namespace_is_same_v = ::std;

namespace _kotek_hd_container_namespace_forward = ::std;
...
/// @tparam Type
template <class Type>
using _kotek_hd_il_t = ::std::initializer_list<Type>;

template <typename Type>
using hybrid_deque_container = _kotek_hd_container_namespace_pmr::deque<Type>;

#elif defined(KOTEK_USE_STD_LIBRARY)

namespace _kotek_hd_container_namespace = ::std;
...
// Returns buffer size for std::pmr::deque. deque is a block-based container:
// it has no reserve() and no contiguous storage, so unlike hybrid_vector the
// preallocated buffer must also cover the block bookkeeping, not only the
// element payload.
template <typename Type>
constexpr kun_ktk size_t _kotek_hd_buffer_size(
...
template <typename Type, kun_ktk size_t ElementCount, bool Realloc,
	kun_ktk size_t _kotek_hd_Size
#ifdef KOTEK_DEBUG
	= _kotek_hd_buffer_size<Type>(ElementCount) * 2
#else
	= _kotek_hd_buffer_size<Type>(ElementCount)
#endif
	>
class hybrid_deque_impl
{
	static_assert(ElementCount == 0 ? Realloc : true,
		"you must set Realloc=true if you passed ElementCount==0");

public:
	using container_type = typename hybrid_deque_container<Type>;
...
public:
	hybrid_deque_impl() : mem() {}

	explicit hybrid_deque_impl(size_type count) : mem(count) {}

	hybrid_deque_impl(size_type count, const Type& value) : mem(count, value)
	{
	}

	template <class InputIt>
	hybrid_deque_impl(InputIt first, InputIt last) : mem(first, last)
	{
	}

	template <typename Type2, kun_ktk size_t ElementCount2, bool Realloc2,
		typename = _kotek_hd_container_namespace_enable_if::enable_if_t<
			(ElementCount >= ElementCount2 || Realloc == true) &&
			_kotek_hd_container_namespace_is_same_v::is_same_v<Type, Type2>>>
	hybrid_deque_impl(
		const hybrid_deque_impl<Type2, ElementCount2, Realloc2>& other) :
		mem(other)
	{
	}

	hybrid_deque_impl(const hybrid_deque_impl& other) : mem(other) {}
	...
	template <typename Type2, kun_ktk size_t ElementCount2, bool Realloc2,
		typename = _kotek_hd_container_namespace_enable_if::enable_if_t<
			...
			_kotek_hd_container_namespace_is_same_v::is_same_v<Type, Type2>>>
	hybrid_deque_impl(
		hybrid_deque_impl<Type2, ElementCount2, Realloc2>&& other) : mem(other)
	{
	}

	hybrid_deque_impl(hybrid_deque_impl&& other) noexcept : mem(other) {}

	hybrid_deque_impl(_kotek_hd_il_t<Type> init) : mem(init) {}
...
public:
	...
	reference emplace_back(Args&&... args)
	{
		return mem.con.emplace_back(
			_kotek_hd_container_namespace_forward::forward<Args>(args)...);
	}
	...
	iterator emplace(const_iterator pos, Args&&... args)
	{
		return mem.con.emplace(
			pos, _kotek_hd_container_namespace_forward::forward<Args>(args)...);
	}
	...
	{
		return mem.con.insert(pos, _kotek_hd_container_namespace::move(value));
	}
	...
	iterator insert(const_iterator pos, _kotek_hd_il_t<Type> ilist)
	{
	...
	{
		mem.con.push_back(_kotek_hd_container_namespace::move(value));
	}
	void push_front(const Type& value) { mem.con.push_front(value); }
	void push_front(Type&& value)
	{
		mem.con.push_front(_kotek_hd_container_namespace::move(value));
	}
	...
	void swap(hybrid_deque_impl& other)
	{
		// std doesn't support swapping polymorphic allocators (C++20);
		// no exceptions in this codebase — fail loudly instead
		KOTEK_ASSERT(false,
			"hybrid_deque_impl::swap is not supported with polymorphic "
			"allocators (C++20)");
		//	_kotek_hd_container_namespace_swap::swap(mem.con, other.mem.con);
	}
	...
	void assign(_kotek_hd_il_t<Type> ilist) { return mem.con.assign(ilist); }

	allocator_type get_allocator() const { return mem.con.get_allocator(); }

	hybrid_deque_impl& operator=(const hybrid_deque_impl& other)
	{
		mem.con.operator=(other.container());
		return *this;
	}
	hybrid_deque_impl& operator=(const container_type& other)
	{
		mem.con.operator=(other);
		return *this;
	}

	hybrid_deque_impl& operator=(hybrid_deque_impl&& other)
	{
		mem.con.operator=(
			_kotek_hd_container_namespace::move(other.container_move()));
		return *this;
	}

	hybrid_deque_impl& operator=(container_type&& other)
	{
		mem.con.operator=(_kotek_hd_container_namespace::move(other));
		return *this;
	}

	hybrid_deque_impl& operator=(_kotek_hd_il_t<value_type> ilist)
	{
...
public:
	...
	{
		return _kotek_hd_container_namespace::move(mem.con);
	}
...
private:
	struct layout_prealloc_t
	{
		layout_prealloc_t() :
			pool{(ElementCount == 0) ? nullptr : buf,
				(ElementCount == 0) ? 0 : _kotek_hd_Size,
				Realloc
					? _kotek_hd_container_namespace_pmr::get_default_resource()
					: _kotek_hd_container_namespace_pmr::
		...
		template <typename Type2, kun_ktk size_t ElementCount2, bool Realloc2,
			typename = _kotek_hd_container_namespace_enable_if::enable_if_t<
				(ElementCount >= ElementCount2 || Realloc == true) &&
				_kotek_hd_container_namespace_is_same_v::is_same_v<Type,
					Type2>>>
		layout_prealloc_t(
			const hybrid_deque_impl<Type2, ElementCount2, Realloc2>& other) :
			pool{(ElementCount == 0) ? nullptr : buf,
		...
		layout_prealloc_t(const hybrid_deque_impl<Type, ElementCount, Realloc,
			_kotek_hd_Size>& other) :
			pool{(ElementCount == 0) ? nullptr : buf,
				(ElementCount == 0) ? 0 : _kotek_hd_Size,
				Realloc
					? _kotek_hd_container_namespace_pmr::get_default_resource()
		...
		template <typename Type2, kun_ktk size_t ElementCount2, bool Realloc2,
			typename = _kotek_hd_container_namespace_enable_if::enable_if_t<
				...
				_kotek_hd_container_namespace_is_same_v::is_same_v<Type,
					Type2>>>
		layout_prealloc_t(
			hybrid_deque_impl<Type2, ElementCount2, Realloc2>&& other) :
			pool{(ElementCount == 0) ? nullptr : buf,
				(ElementCount == 0) ? 0 : _kotek_hd_Size,
				Realloc
					? _kotek_hd_container_namespace_pmr::get_default_resource()
					: _kotek_hd_container_namespace_pmr::
						  null_memory_resource()},
			con{_kotek_hd_container_namespace::move(other.container_move()),
				&pool}
		{
		}

		layout_prealloc_t(
			hybrid_deque_impl<Type, ElementCount, Realloc, _kotek_hd_Size>&&
				other) noexcept :
			pool{(ElementCount == 0) ? nullptr : buf,
				(ElementCount == 0) ? 0 : _kotek_hd_Size,
				Realloc
		...
		layout_prealloc_t(_kotek_hd_il_t<Type> init) :
			pool{(ElementCount == 0) ? nullptr : buf,
				(ElementCount == 0) ? 0 : _kotek_hd_Size,
				Realloc
					? _kotek_hd_container_namespace_pmr::get_default_resource()
		...
		unsigned char buf[_kotek_hd_Size == 0 ? 1 : _kotek_hd_Size];
		_kotek_hd_container_namespace_pmr::monotonic_buffer_resource pool;
		container_type con;
	};
	...
	struct layout_no_prealloc_t
	{
		...
		layout_no_prealloc_t(const hybrid_deque_impl<Type, ElementCount,
			Realloc, _kotek_hd_Size>& other) : con{other.container()}
		{
		}
		...
		layout_no_prealloc_t(_kotek_hd_il_t<Type> init) : con{init} {}

		container_type con;
	};

	using layout_t = _kotek_hd_container_namespace_conditional::conditional_t<
...
using hybrid_std_deque = hybrid_deque_impl<Type, 0, true>;

template <typename Type, kun_ktk size_t ElementCount>
using hybrid_deque = hybrid_deque_impl<Type, ElementCount, true>;

template <typename Type, kun_ktk size_t ElementCount>
using static_hybrid_deque = hybrid_deque_impl<Type, ElementCount, false>;

KOTEK_END_NAMESPACE_KTK

template <typename Type, kun_ktk size_t NotInUsed = 0>
using hybrid_std_deque_t = kun_ktk hybrid_std_deque<Type, NotInUsed>;

template <typename Type, kun_ktk size_t ElementCount>
using hybrid_deque_t = kun_ktk hybrid_deque<Type, ElementCount>;

template <typename Type, kun_ktk size_t ElementCount>
using static_hybrid_deque_t = kun_ktk static_hybrid_deque<Type, ElementCount>;
...
struct uses_allocator<KOTEK_USE_NAMESPACE_KOTEK KOTEK_USE_NAMESPACE_KTK
						  hybrid_deque_impl<Type, ElementCount, Realloc, Size>,
	Alloc> : false_type
{
};
} // namespace std
